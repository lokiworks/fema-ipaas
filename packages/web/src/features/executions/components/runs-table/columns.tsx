...
type RunsTableColumnsProps = {
  data: SeekPage<Execution> | undefined;
  ...
  onViewError: (run: Execution) => void;
  onViewRun: (run: Execution) => void;
...
export const runsTableColumns = ({
...
}: RunsTableColumnsProps): ColumnDef<RowDataWithActions<Execution>>[] => [
  {
    id: 'select',
    accessorKey: 'select',
    size: 40,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const { variant, Icon } = executionUtils.getStatusIcon(status);
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (isNil(failedStep)) {
        if (status === ExecutionStatus.INTERNAL_ERROR && canViewInternalError) {
          return (
            <div className="text-left">
              <Tooltip>
