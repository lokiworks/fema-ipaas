...
> **Always pass `auth`.** Every `Property.Dropdown`, `Property.MultiSelectDropdown`, and `Property.DynamicProperties` whose `options`/`props` callback reads `auth` MUST set `auth: <connectorAuth>` (e.g. `auth: myAppAuth`). Without it, `auth` is `undefined` in the callback and the dropdown can never load. Import the auth object from `../auth` (relative to `src/lib/*`) — never from a re-export on `src/index.ts`. See `packages/connectors/community/github/src/lib/common/index.ts` for the real pattern.
...
**Real example:** `packages/connectors/community/github/src/lib/common/index.ts` -- see `repositoryDropdown`, `issueDropdown`, `labelDropDown`
...
For the `ConnectorCategory` values used in `createConnector({ categories: [...] })`, see `connector-types.md` — it lists every category with guidance on which to pick. (Categories aren't a prop type, so they live there rather than here.)
