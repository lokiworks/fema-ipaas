import { ProjectType, ProjectWithLimits } from '@fema-ipaas/shared';
...
export default function ProjectsPage() {
  ...
  const { data: allProjects } =
    projectCollectionUtils.useAllTenantProjects(filters);
  ...
  const columns = useMemo(() => projectsTableColumns(), []);
  ...
  const errorToastMessage = (error: unknown): string | undefined => {
    if (validationUtils.isValidationError(error)) {
      ...
      switch (error.response?.data?.params?.message) {
        case 'PROJECT_HAS_ENABLED_WORKFLOWS':
          return t('Project has enabled workflows. Please disable them first.');
        case 'ACTIVE_PROJECT':
          return t(
