...
export const addTag = createAction({
  ...
  aiMetadata: { description: 'Attaches a label to the workflow run that is currently executing, so that run can later be located or grouped by that tag when listing runs through the FEMA Integration Tenant API. Pick this to mark a run with a business identifier or a branch it took (e.g. a customer id, "retry", "escalated"); it affects only the current run and the tag is surfaced through the API rather than the run UI. Requires a tag name; idempotent, since the run holds a de-duplicated set of tags, so adding the same name again leaves the same end state.', idempotent: true },
