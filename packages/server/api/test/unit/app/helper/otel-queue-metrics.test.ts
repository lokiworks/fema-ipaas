...
vi.mock('@fema-ipaas/server-utils', async (importOriginal) => {
    const original = await importOriginal<typeof import('@fema-ipaas/server-utils')>()
    return {
        ...original,
        safeHttp: {
            axios: { post: postMock },
...
describe('otelQueueMetrics.push', () => {
    beforeEach(() => {
        ...
        delete process.env.FEMA_OTEL_QUEUE_METRICS_ENABLED
    ...
    it('does nothing when FEMA_OTEL_QUEUE_METRICS_ENABLED is not true', async () => {
    ...
    it('does nothing when no endpoint is configured', async () => {
        process.env.FEMA_OTEL_QUEUE_METRICS_ENABLED = 'true'
