...
Templates are a library of reusable flow (and table) blueprints users can browse, import, and build on. Before saving, flows inside a template are validated and their connector names extracted into a searchable `connectors` array.

### Entities & services
- **Template** entity: name, summary, description, type, status, platformId (nullable), flows (jsonb `FlowVersionTemplate[]`), tables, tags, categories (indexed text[]), connectors (indexed text[]).
