...
icon: 'Joystick'
description: 'Learn how to control workflow execution from inside a connector'
---

Workflow Controls let an action change the shape of the run — stop it early, send an intermediate HTTP response, or **pause the workflow and resume later** when an external signal arrives. All of these are exposed on the `ctx` parameter of the action's `run` method.

## Stop Workflow

Stop the workflow and (optionally) return a response to the webhook trigger that started it.
...
```typescript
import { ExecutionType } from '@fema-ipaas/shared';
...
Create a `WEBHOOK` waitpoint and expose its resume URL — the workflow will resume whenever that URL is called.
...
`buildResumeUrl` takes an optional `sync: true` to return the caller a synchronous response produced by the remainder of the workflow, and a `queryParams` object that is carried through to `ctx.resumePayload.queryParams` on resume.

### Respond immediately and wait for the next webhook

Pause the workflow **and** immediately reply to the webhook trigger — useful for "we got your submission, we'll call you back" patterns. Pass `responseToSend` and your HTTP response is sent right away; the workflow then sits paused until the returned URL is called.
...
`resumeDateTime` is capped by the server's `FEMA_PAUSED_WORKFLOW_TIMEOUT_DAYS` setting; the engine throws `PausedWorkflowTimeoutError` if you ask for a longer delay.
...
<Warning>
  **Deprecated:** older connectors use `ctx.run.pause({ pauseMetadata: { type: PauseType.WEBHOOK | PauseType.DELAY, ... } })` together with `ctx.generateResumeUrl(...)`. That V0 API is kept for backwards compatibility with in-flight paused runs and will be removed. New actions must use `ctx.run.createWaitpoint` + `ctx.run.waitForWaitpoint`.
</Warning>
