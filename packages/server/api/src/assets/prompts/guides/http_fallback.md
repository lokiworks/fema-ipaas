...
If anything still fails after one corrected attempt, call `ap_get_connector_props('@fema-ipaas/connector-http', 'send_request')` (passing your current `body_type`) to resolve the dynamic `body` sub-fields, fix once, and report if it still won't go through — do not re-send the same request repeatedly.
