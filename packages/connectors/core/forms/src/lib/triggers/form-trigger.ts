...
If **Wait for Response** is enabled, use **Respond on UI** in your workflow to provide a response back to the form.
`;
...
export const onFormSubmission = createTrigger({
  ...
  description: 'Trigger the workflow by submitting a form.',
  aiMetadata: {
    description: 'Fires once each time a person submits the hosted web form generated by this trigger, carrying the value of every field configured on it. Represents one-shot human input as a set of structured fields, unlike the sibling Chat UI trigger that fires per message in a multi-turn conversation. When "Wait for Response" is enabled the submitter is held waiting until the workflow calls the Respond on UI action.',
  },
  props: {
