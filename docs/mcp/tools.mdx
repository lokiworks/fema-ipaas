...
Read the full untruncated settings of any step, including the trigger: connector input, action/trigger name, loop items, router branches, and error handling options. Use this to see a step's current configuration before updating it — `ap_flow_structure` truncates connector input.
...
Find connector triggers (the event that starts a flow) by describing when the flow should run (e.g. *"when a new row is added to a Google Sheet"*). Same behavior and result shape as `ap_search_actions`, returning trigger names instead of action names.
...
| `continueOnFailure` | boolean | No | For CODE/CONNECTOR steps: continue flow if this step fails (default: false) |
...
| `continueOnFailure` | boolean | No | For CODE/CONNECTOR steps: continue flow if this step fails |
