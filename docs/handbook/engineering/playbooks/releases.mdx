...
All changes workflow through a staging environment before reaching production. There are no release branches — everything merges to `main`.

## How It Works

Five separate workflows cover the full delivery lifecycle:

1. **Merge to `main`** (Mon–Thu, 9 AM–5 PM UTC) — `continuous-delivery-stg.yml` automatically builds a Docker image and deploys to **staging** (`stg.fema.local`).
...
|---|---|---|
| Preview | `branch-name.fema.local` | Per-PR ephemeral environment for review. *Requires the `preview` label on the PR.* |
| Staging | `stg.fema.local` | Internal testing, content team daily use |
| Canary | `canary.fema.local` | Daily cut from `main`; catches issues before they reach production |
| Production | `fema.local` | Live customers |
...
| Trigger | What happens |
|---|---|
| **Scheduled (Mondays 9 AM UTC)** | Publishes a GitHub release with changelog, creates a git tag, and re-tags `ghcr.io/lokiworks/fema-ipaas-cloud:release-candidate` as `lokiworks/fema-ipaas:X.Y.Z` + `latest` on Docker Hub and GHCR. |
