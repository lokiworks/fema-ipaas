...
/**
 * Migration script for custom connectors from Nx to Turbo.
 *
 * This script migrates a custom connector (or all connectors in a directory) from the
 ...
 *
 * If no path is provided, it scans packages/connectors/custom/ for all connectors.
 */

import * as fs from 'fs';
import * as path from 'path';

const CUSTOM_CONNECTORS_DIR = path.resolve(__dirname, '../../packages/connectors/custom');

function getRelativeRoot(connectorDir: string): string {
  const connectorsIndex = connectorDir.indexOf('/packages/connectors/');
  if (connectorsIndex === -1) {
    throw new Error(`Unexpected connector directory structure: ${connectorDir}`);
  }
  const relative = path.relative(connectorDir, connectorDir.substring(0, connectorsIndex));
  return relative;
}

function migrateConnector(connectorDir: string): void {
  const connectorName = path.basename(connectorDir);
  const relativeRoot = getRelativeRoot(connectorDir);

  console.log(`\nMigrating connector: ${connectorName}`);
  ...
  const tsconfigLibPath = path.join(connectorDir, 'tsconfig.lib.json');
  ...
  const tsconfigPath = path.join(connectorDir, 'tsconfig.json');
...
function findConnectorDirs(baseDir: string): string[] {
...
if (args.length > 0) {
  // Migrate a specific connector path
  const connectorPath = path.resolve(args[0]);
  if (!fs.existsSync(connectorPath)) {
    console.error(`Error: Path not found: ${connectorPath}`);
    process.exit(1);
  }
  migrateConnector(connectorPath);
} else {
  // Migrate all custom connectors
  console.log(`Scanning ${CUSTOM_CONNECTORS_DIR} for connectors...`);
  const connectorDirs = findConnectorDirs(CUSTOM_CONNECTORS_DIR);

  if (connectorDirs.length === 0) {
    console.log('No custom connectors found to migrate.');
    process.exit(0);
  }

  console.log(`Found ${connectorDirs.length} connector(s) to check.\n`);
