...
title: 'Flow Control'
icon: 'Joystick'
description: 'Learn how to control flow execution from inside a connector'
---
...
`resumeDateTime` is capped by the server's `FEMA_PAUSED_FLOW_TIMEOUT_DAYS` setting; the engine throws `PausedFlowTimeoutError` if you ask for a longer delay.
