...
- Connectors use SDK helpers `storage.get` / `storage.put` / `storage.delete`.
...
- `value` is nullable; a connector can store `null`/`undefined`. Values run through `sanitizeObjectForPostgresql` before storage.
...
- `packages/server/engine/src/lib/connector-context/store.ts` — engine side that actually calls `/v1/store-entries`, builds the scoped key, maps errors
