...
**1. Clone FEMA Integration Platform repository.**

Use the command line to clone FEMA Integration Platform repository:
...
**4. Run FEMA Integration Platform.**
...
ngrok http 8080
```
3. Replace `FEMA_FRONTEND_URL` environment variable in `.env` with the ngrok url.
...
Review the [configurations guide](/install/reference/environment-variables) to make any necessary adjustments.
</Warning>

## Upgrading

To upgrade to new versions, which are installed using docker compose, perform the following steps. First, open a terminal in the fema repository directory and run the following commands.
...
<Warning>
Please review breaking changes in the [changelog](../reference/breaking-changes).
</Warning>
