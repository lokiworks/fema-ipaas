...
| Layer | Warm |
|---|---|
| Provision (flow bundle + connector + engine, all disk-cache hits) | 0.5 ms |
...
Warm provisioning and boot are effectively free — the connector cache is on local disk and the engine process is reused, so essentially the entire worker-busy time is the flow run itself. That figure grows with the fleet (167 ms at 40 workers, 146 ms at 80, 167 ms at 120, 181 ms at 160), which is the per-flow stretch behind the falling per-worker rate above.
