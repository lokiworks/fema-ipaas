...
1. A **running local dev instance** (`npm start` / `npm run dev`). Dev connectors load from each connector's built `dist/` — see [capture-recipes.md](./capture-recipes.md#dev-connector-reload) if a connector doesn't appear.
2. A **real, active connection** for the target connector — OAuth sign-in, API key, or whatever the connector's auth type requires. The user provides credentials.
3. A connector not yet loaded as a dev connector gets its folder name appended to `FEMA_DEV_CONNECTORS`.

**Ask the user** for the connector(s) and the connection to use before starting.

## Workflow

### Step 1 — Scope the connector
List the connector's actions and triggers (`packages/connectors/community/<connector>/src/lib/{actions,triggers}`). For each, decide whether it gets a schema using the table below.
...
Write the schema in `packages/connectors/community/<connector>/src/lib/output-schemas.ts` (create the file if absent). Full field reference, formats, labels, and wiring in [schema-reference.md](./schema-reference.md). The essentials:
...
Resolve every field's `value ?? key` against the captured JSON **at the correct scope** — top-level against the root, `children` against the parent object, `listItems` against one array item. A path that doesn't resolve is a dead field; re-capture if the shape is ambiguous. For a connector with many schemas, verify each one adversarially — one sub-agent per schema, given only the schema and its captured payload, asked to find any path that fails to resolve.
...
- **Bump the connector's patch version** in its `package.json` (every touched connector) — this is what forces cloud/self-hosted registries to re-ingest the fresh metadata.
- Rebuild the connector and reload the dev instance ([capture-recipes.md](./capture-recipes.md#dev-connector-reload)); confirm the friendly tree renders in the builder.
- Run `npm run lint-dev` (or `npx turbo run lint --filter=@fema-ipaas/connector-<name>`). Typecheck must be clean.
...
- [ ] Each touched connector's patch version bumped; build + `lint-dev` green; friendly tree verified in the builder.

## Note: how the schema reaches the builder
`outputSchema` ships as part of the served connector metadata (dev connectors from `dist/`, published connectors from the registry). If a schema doesn't appear in the builder, confirm the connector's patch version was bumped and the connector was rebuilt + reloaded — that's the served metadata refreshing. Legacy servers older than #13983 stripped `outputSchema` during registry ingestion; irrelevant for current builds.

## Related
- `connector-builder` skill — building connectors and the `output-quality.md` reference (shaping `run()` return values for table-readiness) complements this skill, which describes an *existing* return.
