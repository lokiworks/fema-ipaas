...
export const appConnectionService = (log: FastifyBaseLogger) => ({
    ...
    async replace(params: ReplaceParams): Promise<void> {
        ...
        if (sourceAppConnection.connectorName !== targetAppConnection.connectorName) {
            throw new PlatformError({
                code: ErrorCode.VALIDATION,
                params: {
    ...
    async getOwners({ projectId: _projectId, platformId }: { projectId: ProjectId, platformId: PlatformId }): Promise<AppConnectionOwners[]> {
