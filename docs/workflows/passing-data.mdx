...
Any FEMA Integration Platform workflow is a vertical diagram that **starts with a trigger step** followed by **any number of action steps**.

Steps are connected vertically. Data workflows from parent steps to the children. Children steps have access to the output data of the parent steps.
...
This workflow has 3 steps, they can access data as follows:
...
- **Step 3** can access data produced by Steps 1 and 2 as they're its parent steps. This step can produce data but since it's the last step in the workflow, it can't be used by other ones.
...
We require you to test steps before accessing their data. This approach protects you from selecting the wrong data and breaking your workflows after publishing them.
...
Follow the instructions in the Generate Sample Data panel to know how your step should be tested. Some triggers will also let you Use Mock Data, which will generate static sample data from the connector. We recommend that you test the step instead of using mock data.
...
instead of loading it fully into memory, so your workflow can move files larger than the
...
[Large File Streaming](/build-connectors/connector-reference/large-file-streaming) for the list of
...
The `step_slug` can be found by moving your cursor over any of your workflow steps, it will show to the right of the step.