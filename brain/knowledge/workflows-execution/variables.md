...
Project-scoped, encrypted secret values (API keys, tokens) users create once and reference in any workflow input via `{{variables['NAME']}}`. Stored in a dedicated `variable` table, fully separate from `connection`. Ships in every edition (no plan flag).
