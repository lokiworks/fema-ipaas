...
The value is `{ preset, after?, before? }`. Resolve it to concrete ISO bounds inside `run()` with `dateRangeUtils.resolve` — relative presets resolve against "now", so recurring workflows roll the window forward:

```typescript
import { dateRangeUtils } from '@fema-ipaas/connectors-framework';
...
<Tip>
  When accessing the Connector auth, be sure to use exactly `auth` as it is
...
This is a property that lets you inject JS code into the frontend and manipulate the DOM of this content however you like, it is extremely useful in case you are [embedding](/embedding/overview) FEMA Integration Platform and want to have a way to communicate with the SaaS embedding it.
...
| isEmbedded | `boolean` | The flag that tells you if the code is running inside an [embedded instance](/embedding/overview) of FEMA Integration Platform |
| projectId | `string` | The project ID of the workflow the step that contains this property is in |
...
- **Must** mark your connector `minimumSupportedRelease` property to be at least `0.58.0` after introducing this property to it.
...
- If you would like to know more about how to setup communication between FEMA Integration Platform and the SaaS that's embedding it, check the [window postMessage API](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage).
