...
Platform-level reporting on automation usage — daily run counts, active workflows/users, and time-saved estimates — powering the "Impact" dashboard (`/impact`). Gated by the `analyticsEnabled` plan flag (EE/Cloud only, not in CE).

### Entities & services

- **PlatformAnalyticsReport**: cached entity holding `runs` (daily `{workflowId, day, runs}`), `workflows` (enabled-workflow metadata), `users`, plus `outdated` flag and `cachedAt`.
...
- `getOrGenerateReport` serves a cached report (5-min TTL) filtered by time period. `refreshReport` runs under a distributed lock (400s), queries users + enabled workflows + daily run counts (**PRODUCTION runs only**), and merges incrementally.
- **Time saved**: `minutesSaved = runs × workflow.timeSavedPerRun` (per-workflow estimate in minutes, editable by the workflow owner in the Details drill-down).
- Connectors analytics (daily cron 12:00 UTC): for each enabled workflow, extract connector steps, count unique projects per connector, write `connectorMetadata.usage = projectCount`.
...
- `packages/web/src/app/routes/impact/lib/` — aggregation and filter helpers; `impactRunsUtils.sumRunsByWorkflow` is the single run-count aggregation every consumer routes through
