...
const baseVersion = (trigger: WorkflowVersion['trigger']): WorkflowVersion => ({
    id: 'fv-1',
    displayName: 'fixture',
...
const triggerWithNoNext = (overrides: Partial<WorkflowVersion['trigger']['settings']> = {}): WorkflowVersion['trigger'] => ({
...
describe('migrateV21StepOutputNesting', () => {
    ...
    describe('P. migration-level integration', () => {
        ...
        it('P2. round-trip preserves non-expression fields and rewrites expressions', async () => {
            ...
            expect(result.workflowId).toBe(version.workflowId)
        ...
        it('P3. multi-step workflow with nested router children', async () => {
            const version = baseVersion({
                ...triggerWithNoNext(),
                nextAction: {
                    type: WorkflowActionType.ROUTER,
