...
Flow runs have a maximum log size (default **25 MB**). When a run gets close to this limit, FEMA Integration Platform tries to keep it under the cap by truncating large step **inputs** — you'll see `(truncated)` in place of the original value.
...
Increase the flow run log size limit by setting the `FEMA_MAX_EXECUTION_LOG_SIZE_MB` environment variable:
