...
icon: 'water'
description: "Move large files through a workflow without buffering them in memory"
---

Large file streaming lets a connector process a big file by passing it through as a stream —
...
destination. Because the file is never fully buffered, a workflow can move files far larger
than the worker's memory budget.

## Why it matters

A worker runs your connector code inside a sandbox with a bounded memory budget (about 1&nbsp;GB,
...
Streaming avoids this: the bytes workflow through as a Node `Readable`, roughly 5&nbsp;MB at a
time, so no process ever holds the whole file. The transfer is transparent — there are no
extra "chunk" steps in your workflow; a streaming action looks and behaves like any other.

<Warning>
**Requires S3 file storage.** Full streaming only works when file storage is set to S3
(`FEMA_FILE_STORAGE_LOCATION=S3`). With the default database (`DB`) storage a stream **can't**
...
## Connectors that support streaming
...
| Connector | Streams in | Streams out |
|---|---|---|
...
More connectors are being enabled over time. Actions not listed here still work — they buffer
...
| Connector | One-request cap | Above it |
|---|---|---|
...
If you are [building a connector](/build-connectors/building-connectors/overview), you can stream on both sides: read an
...
`StreamingFile` instead of an `ConnectorFile`:
...
<Tip>
`Property.File()` without `streaming` is unchanged — it still resolves to an `ConnectorFile` with a
...
- **Writing into storage is capped.** A stream passed to `ctx.files.write` is counted against
  `FEMA_MAX_FILE_SIZE_MB` (Cloud: 10&nbsp;MB, self-hosted default: 25&nbsp;MB) *while* the bytes
  workflow; exceeding it aborts the transfer and fails the step. See
  [Limits](/install/reference/limits).
- **Reading a streamed input is not capped.** A `streaming: true` file input has no
  `FEMA_MAX_FILE_SIZE_MB` ceiling — that is deliberate, since the point of the feature is to move
...
- **`httpClient` does not retry a stream body at all.** The retry loop in `connectors-common`
  ...
  `0`. This applies to **any** connector sending a stream through `httpClient`, not just file
