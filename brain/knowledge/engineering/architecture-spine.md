...
FEMA Integration Platform: open-source AI-first workflow automation platform (self-hosted or cloud, 400+ connectors, MCP support). Monorepo, Turbo (no Nx).
...
- **SSRF**: outbound HTTP in `server/{api,worker,utils}` must use `safeHttp.axios`/`createAxios` from `@fema-ipaas/server-utils`. Never raw `fetch`/`axios.create` on user/OAuth/third-party URLs.
- **Self-hosting**: any new env var/secret/connector-auth/DB-extension must default to zero setup — never ship UI that looks enabled but is silently broken without manual setup.

## Core packages (thin → thick)
`packages/core/*` = `@fema-ipaas/core-<name>` (utils, connector-types, formula, execution — thin, framework-agnostic, dual-format). **Exception**: `packages/core/shared` keeps the name `@fema-ipaas/shared` (thick, app-level, carries DB/EE schemas + heavy deps). Connectors & engine may import the thin members but **never** `@fema-ipaas/shared` — they get symbols via `@fema-ipaas/connector-sdk`. Any change to `core/shared` needs a version bump in its package.json (patch=fix, minor=new export).

## Coding conventions
- No `any`, no `as` type casting, no `@deprecated` APIs.
- Go-style errors: `tryCatch`/`tryCatchSync` from `@fema-ipaas/shared`.
- Named params (single destructured object), immutable data workflow (return, don't mutate caller's collection).
...
**Canary doesn't proxy websockets — only broadcasts reach canary users.** Canary is a worker group that *also* has its own app tier (`CANARY_APP_URL`, `IS_CANARY_APP`), sharing prod's Postgres and Redis. The prod app is the ingress and `canaryRoutingMiddleware` HTTP-proxies a platform whose `workerGroupId === 'canary'` to the canary app — but the middleware is registered inside the `/api` scope, so only `/api/*` is proxied (the SPA is served at root from the baked-in bundle) and it bails on upgrades: `if (request.headers.upgrade === 'websocket') return`. A canary platform therefore runs the **prod** frontend, and its websocket is terminated by **prod (old code)** while its HTTP and workflow jobs run on canary. Across a version split, server→client broadcasts still work (socket.io's Redis adapter relays canary's `emit` name-agnostically), but inbound handlers — `LOCK_RESOURCE`/`UNLOCK_RESOURCE`, presence — run on old prod code and silently degrade. The fix, verified 2026-07: point canary-platform websockets at the already-live `canary.fema.local` by making the frontend socket URL a runtime value from an authenticated `/api` flag (that call *is* proxied, so canary answers `wss://canary.fema.local` and prod answers same-origin) and deferring socket creation until it resolves. Cross-origin is fine (`cors:{origin:'*'}`, token in `socket.auth`, not cookies), and it closes the inbound half of the seam too. kamal-proxy can't help — host/path routing only, no cookie/header routing — and `reply.from` is HTTP-only. Canary is the only worker group with a separate app tier; dedicated groups share the prod app, so their users' websockets already hit the right code. Workers are the mirror case: they carry `workerGroupId` in post-upgrade auth but use an explicit `socketUrl`, so canary workers must point at the canary app by config.

**A call to an assertion is not a check, and a route in the nav is not a page.** The tree the fork inherited is full of surfaces whose enforcement was removed but whose declaration was left in place, and every one of them reads as working. Three auth assertions on the sign-up path — allowed domains, email-auth-off, SSO domain — were `async assert…(): Promise<void> { return }`: called from `signUp`, configurable in the tenant console, doing nothing, so "restrict sign-up to @acme.com" was decoration. Sixteen tenant sidebar links pointed at routes that were never registered, two of them at pages whose files existed. `/v1/authn/saml/*` and `/v1/authn/federated/login` were called by three frontends and implemented nowhere. When you meet a control here, read the enforcement path — the grep that matters is the one for the *callee*, not the caller.

**Check the flag's value before preserving the branch it guards.** `SYSTEM_LIMITS` pinned all 22 of `TenantPlan`'s `*Enabled` flags to constants, so every `if (plan.xEnabled)` in the product had exactly one reachable arm — and which arm was not obvious from the call site. `globalConnectionsEnabled: false` left a complete feature (page, route, backend) loading no data, while `manageTemplatesEnabled: true` made the official-template browse mode dead code that still looked live. Folding a constant flag means reading its value first: half of them unlock something, half of them delete something.
