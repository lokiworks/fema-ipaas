...
function migrateConnector({ connectorPath, dryRun }: MigrateConnectorParams): MigrateReport {
    const repointedFiles: string[] = []
    const srcDir = join(connectorPath, 'src')
    ...
    const manifestChanged = migrateManifest({ connectorPath, dryRun })
    const eslintChanged = migrateEslintConfig({ connectorPath, dryRun })
...
function migrateManifest({ connectorPath, dryRun }: MigrateConnectorParams): boolean {
    const manifestPath = join(connectorPath, 'package.json')
...
function migrateEslintConfig({ connectorPath, dryRun }: MigrateConnectorParams): boolean {
    const configPath = join(connectorPath, '.eslintrc.json')
...
export const migrateConnectorUtils = { migrateConnector }

export type MigrateConnectorParams = {
