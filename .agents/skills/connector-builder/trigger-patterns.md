...
Use when the API has no webhook support. FEMA Integration Platform polls every ~5 minutes.
...
import { DedupeStrategy, Polling, pollingHelper, httpClient, HttpMethod, AuthenticationType } from '@fema-ipaas/connector-common';
...
**Real example:** `packages/connectors/community/airtable/src/lib/trigger/new-record.trigger.ts`
...
export const newRecordWebhookTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    const response = await httpClient.sendRequest<{ id: string }>({
      ...
      body: {
        url: context.webhookUrl,         // FEMA Integration Platform provides this
...
**Real example:** `packages/connectors/community/stripe/src/lib/trigger/new-customer.ts`
...
```typescript
import { WebhookHandshakeStrategy } from '@fema-ipaas/connector-sdk';
