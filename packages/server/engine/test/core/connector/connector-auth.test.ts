...
describe('connector-auth callMethod', () => {
    ...
    it('unwraps the connector value from the { result, hooks } wrapper', async () => {
        ...
        const result = await connectorAuth.callMethod({ operation: operationFor(SECRET_TEXT_VALUE), authValueType: AppConnectionType.SECRET_TEXT, methodPath: ['validate'] })
    ...
    it('unwraps the refresh result so access_token is reachable', async () => {
        ...
        const result = await connectorAuth.callMethod({ operation: operationFor(CUSTOM_AUTH_VALUE), authValueType: AppConnectionType.CUSTOM_AUTH, methodPath: ['refresh', 'generate'] })
    ...
    it('passes the resolved path, argument and slash-normalized server url to the runner', async () => {
        ...
        await connectorAuth.callMethod({ operation: operationFor(SECRET_TEXT_VALUE), authValueType: AppConnectionType.SECRET_TEXT, methodPath: ['validate'] })
    ...
    it('selects the indexed auth path when the connector exposes an auth array', async () => {
        ...
        await connectorAuth.callMethod({ operation: operationFor(CUSTOM_AUTH_VALUE), authValueType: AppConnectionType.CUSTOM_AUTH, methodPath: ['validate'] })
    ...
    it('returns called:false with mismatch when the connection value type does not fit the property', async () => {
        ...
        const result = await connectorAuth.callMethod({ operation: operationFor(CUSTOM_AUTH_VALUE), authValueType: AppConnectionType.SECRET_TEXT, methodPath: ['validate'] })
