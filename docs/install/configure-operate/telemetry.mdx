...
# Why Does FEMA Integration Platform need data?
...
To ensure we can continue to improve our product, we have decided to track certain basic behaviors and metrics that are vital for understanding the usage of FEMA Integration Platform.

We have implemented a minimal tracking plan and provide a detailed list of the metrics collected in a separate section.


# What Does FEMA Integration Platform Collect?
...
[Exact Code](https://github.com/lokiworks/fema-ipaas/blob/main/packages/core/shared/src/lib/common/telemetry.ts)


# Opting out?

To opt out, set the environment variable `FEMA_TELEMETRY_ENABLED=false`