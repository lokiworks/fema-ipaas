...
export const jobQueue = (log: FastifyBaseLogger) => ({
    ...
    async add(params: AddJobParams<JobType>): Promise<Job | null> {
        ...
        const queueName = await getQueueName({ tenantId, projectId, jobType: data.jobType }, log)
    ...
    async removeOneTimeJob({ jobId, tenantId, projectId, jobType }: RemoveOneTimeJobParams): Promise<void> {
        const queueName = await getQueueName({ tenantId, projectId, jobType }, log)
        const queue = await ensureQueueExists({ log, queueName })
    ...
    async cancelAndReportNeverStarted({ jobId, tenantId, projectId, jobType }: RemoveOneTimeJobParams): Promise<boolean> {
        const queueName = await getQueueName({ tenantId, projectId, jobType }, log)
        const queue = await ensureQueueExists({ log, queueName })
        const job = await queue.getJob(jobId)
        if (isNil(job)) {
    ...
    async removeAllExecutionJobs({ executionId, tenantId, projectId }: RemoveAllExecutionJobsParams): Promise<void> {
        const queueName = await getQueueName({ tenantId, projectId, jobType: WorkerJobType.EXECUTE_WORKFLOW }, log)
        const queue = await ensureQueueExists({ log, queueName })
...
async function getQueueName({ tenantId, projectId, jobType }: GetQueueNameParams, log: FastifyBaseLogger): Promise<string> {
    if (!isNil(tenantId) && !isNil(projectId) && !isNil(jobType) && PROJECT_GROUP_ROUTABLE_JOB_TYPES.has(jobType)) {
        const projectGroupId = await projectWorkerGroupService(log).getProjectWorkerGroup({ projectId, tenantId })
