...
**Import boundary (enforced per-package, not by folder name):** connectors and the engine may import `@fema/core-utils | core-connector-types | core-formula | core-execution`, but **never** `@fema/shared` (`packages/core/shared`). Connectors get the symbols they need re-exported through `@fema/connector-sdk`.
