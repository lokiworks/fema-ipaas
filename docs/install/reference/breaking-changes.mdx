...
- Only if you build dashboards or alerts on FEMA Integration Platform logs. Update them to the new grouped names, e.g. `runId` → `flowRun.id` and `err` → `error`.
...
This is an important behavior change in how flow run logs are stored and capped, but **no action is required** — the existing default (`FEMA_MAX_FLOW_RUN_LOG_SIZE_MB=50`) is preserved.

- Large step outputs are now offloaded to object storage instead of sitting inline in worker memory, controlled by the new `FEMA_FLOW_RUN_LOG_SLICE_THRESHOLD_KB` env var (default `32`).
- Large step input values are replaced with a `(truncated, original size <N>)` placeholder in the log via the new `FEMA_FLOW_RUN_LOG_INPUT_TRUNCATE_THRESHOLD_KB` env var (default `2`). The step still receives the full value at runtime.
- Runs whose combined step inputs and outputs exceed `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB` now terminate with the new `LOG_SIZE_EXCEEDED` status instead of silently trimming inputs. Offloaded outputs still count their original size against the cap.
...
- Log size configuration has changed: `FEMA_MAX_FILE_SIZE_MB` no longer controls flow run logs. Use `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB` instead.
...
- If you use a custom value for `FEMA_MAX_FILE_SIZE_MB`, be sure to also set `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB` accordingly.
