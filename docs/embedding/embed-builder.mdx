...
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<FEMA Integration Platform_url>`. For example if the prefix is `/automation` and the FEMA Integration Platform url is `/flows` the full url would be `/automation/flows`. |
...
| embedding.hideTables | ❌ | boolean | Hides the Tables UI in the dashboard (tree, filters, create/import buttons) and blocks direct access to the table editor route. The Tables connector used inside flows is unaffected. By default it is false. (added in [0.9.0](./sdk-changelog#04%2F21%2F2026-0-9-0)) |
