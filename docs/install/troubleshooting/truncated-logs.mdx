...
description: "Understanding and resolving truncated workflow run logs"
icon: "file-lines"
---

## Overview

Workflow runs have a maximum log size (default **25 MB**). When a run gets close to this limit, FEMA Integration Platform tries to keep it under the cap by truncating large step **inputs** — you'll see `(truncated)` in place of the original value.
...
3. If the run **still** exceeds the limit after all inputs are truncated — meaning step outputs alone are over the cap — the run fails with `LOG_SIZE_EXCEEDED` and an error like:
   ...
   ```
   Workflow run data size exceeded the maximum allowed size of 25 MB
   ```
...
Increase the workflow run log size limit by setting the `FEMA_MAX_EXECUTION_LOG_SIZE_MB` environment variable:

```bash
FEMA_MAX_EXECUTION_LOG_SIZE_MB=50
```
