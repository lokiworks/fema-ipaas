...
| `brain/wiki/<area>/*.md` | one page per subsystem | When Claude explores that subsystem | Entity schemas, services, data workflows, gotchas |
...
**Good:** "Add a project-level analytics dashboard showing workflow run count, success rate, and average duration for the last 7/30/90 days. Project-scoped, CE feature, visible to users with READ_RUN permission."
...
Tab 3: claude --worktree    # Feature C (connector/integration)
```
...
- [ ] `@fema-ipaas/shared` version bumped (if changed)
</Accordion>
</AccordionGroup>
...
/connector-builder "app name"     # Build or extend a connector
