...
<Note>
**Scope:** the recommended production shape (`SANDBOX_CODE_ONLY`, `FEMA_REUSE_SANDBOX=true`, one flow per worker at 0.5 vCPU / 1 GB), measured on GKE with the cluster autoscaler. Other platforms differ mainly in node-provisioning time.
</Note>
...
A new worker's first flow pays the one-time [cold start](/install/architecture/latency) (~2–3 s, connector install + engine boot); its second run is already warm.
