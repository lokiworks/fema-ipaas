...
export const createNewIssue = createTrigger({
    ...
    name: string, // Unique name across the connector.
    ...
    // Run when the user enable or publish the workflow.

	onEnable: (ctx) => {},
    // Run when the user disable the workflow or
    // the old workflow is deleted after new one is published.
	onDisable: (ctx) => {},
...
<Tip>
  ...
  the array will trigger the workflow to run.
</Tip>

## Context Object

The Context object contains multiple helpful connectors of information and tools that can be useful while developing.
...
// Webhook URL: A unique, auto-generated URL that will trigger the workflow. Useful when you need to develop a trigger based on webhooks.
context.webhookUrl;
