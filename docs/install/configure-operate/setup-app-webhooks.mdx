...
3. Set the following environment variable in your fema environment: 
    ```
    FEMA_APP_WEBHOOK_SECRETS={"@fema-ipaas/connector-slack": {"webhookSecret": "SIGNING_SECRET"}}
    ```
...
Needed for the "New Modal Interaction" trigger and the "Request Approval"/"Request Action" actions (their buttons resume the workflow via this URL).
