...
**Write the value explicitly — do not omit it.** Connector metadata is serialized from the raw action objects with no default injection, so downstream filters only see `audience` when it is physically present in the file. For a normal integration action, write `audience: 'both'`.
...
Renders as a badge in the builder's connector selector (`READ`/`SEARCH`/`WRITE` as quiet grey pills, `DESTRUCTIVE` in red) and is the declared read/write signal for future consumers. Convention: place it right after `name:` in the config object.
