...
describe('codeCache', () => {
    it('compiledStepPath is <codesFolder>/<workflowVersionId>/<stepName>/index.js', () => {
        const folder = uniqueFolder()
        const ref = { workflowVersionId: 'fv1', stepName: 'step_1' }
    ...
    it('writeCompiledStep creates missing parent directories', async () => {
        const folder = uniqueFolder()
        const cache = codeCache(folder)
        const ref = { workflowVersionId: 'never-created', stepName: 'deep_step' }
