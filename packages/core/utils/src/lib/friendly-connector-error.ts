...
const FRIENDLY_CONNECTOR_ERROR_VERSION = 1
...
const isFriendlyConnectorError = (value: unknown): value is FriendlyConnectorError => {
    if (!isObjectRecord(value)) {
        return false
    }
    return value['__apErrorVersion'] === FRIENDLY_CONNECTOR_ERROR_VERSION && typeof value['message'] === 'string'
}

export const formatConnectorError = (error: unknown, options?: FormatConnectorErrorOptions): FriendlyConnectorError => {
    const raw = options?.raw
    const withRaw = (base: FriendlyConnectorError): FriendlyConnectorError => {
    ...
    if (isNil(error)) {
        return withRaw({
            __apErrorVersion: FRIENDLY_CONNECTOR_ERROR_VERSION,
    ...
    if (isFriendlyConnectorError(error)) {
...
export const tryParseFriendlyConnectorError = (value: unknown): FriendlyConnectorError | null => {
    if (isNil(value)) {
        return null
    }
    const candidate = isString(value) ? safeJsonParse(value) : value
    return isFriendlyConnectorError(candidate) ? candidate : null
}
...
export type FriendlyConnectorError = {
    __apErrorVersion: 1
    message: string
    errorName?: string
    status?: number
...
type FormatConnectorErrorOptions = {
    raw?: string
}
