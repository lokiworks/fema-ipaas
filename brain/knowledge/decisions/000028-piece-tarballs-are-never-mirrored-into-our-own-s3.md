...
## Context
Until Aug 2026 `resolve()` checked an S3 mirror first (`pieces/v2/`), and on a miss enqueued `BUNDLE_PIECE` to populate it for next time. That job wrote **whichever source was preferred at cache time** — the CDN artifact when `FEMA_USE_CDN_FOR_BUNDLES` was on, the npm tarball otherwise.
