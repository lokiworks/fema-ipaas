...
description: "Guide for creating database migrations in FEMA Integration Platform"
icon: "database"
---

FEMA Integration Platform uses TypeORM as its database driver in Node.js. We support two database types across different editions of our platform.
...
- Developers creating connectors who want a quick setup
...
<Steps>
  <Step title="Setup FEMA_DB_TYPE">
    Set the `FEMA_DB_TYPE` environment variable to `POSTGRES` after making sure have latest state by running FEMA Integration Platform first.
  </Step>
