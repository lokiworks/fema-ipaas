---
title: "Sandboxing"
description: "Choose the right isolation mode for running workflow code"
icon: "box"
---

Workflow code (Code steps and connector actions) always runs inside a **sandbox** that wraps the engine process. `FEMA_EXECUTION_MODE` decides which sandbox. It is the most important security choice in a self-hosted deployment: it decides whether a malicious workflow is contained to one worker pod or can reach the kernel.
...
<Tip>
**For enterprise deployments, use `SANDBOX_CODE_ONLY` (V8 isolation).** It is the only mode that is both multi-tenant-safe *and* runs as an unprivileged container. It is what FEMA Integration Platform Cloud uses, and what fits inside a standard Kubernetes security baseline.
</Tip>
...
You run FEMA Integration Platform in your own Kubernetes cluster, next to a Salesforce-sync service and a finance-analytics pod. A customer ships a malicious Code step.
...
<Warning>
Only choose `SANDBOX_PROCESS` if you genuinely need arbitrary `npm` packages in Code steps, and run it on a dedicated node pool. A privileged FEMA Integration Platform worker should never share a node with unrelated workloads.
</Warning>
...
Execution mode decides *how* user code runs; `FEMA_NETWORK_MODE` decides *what it can reach*. See [Network Security](./network-security) for the in-process SSRF guard that layers on top of every sandbox mode.
