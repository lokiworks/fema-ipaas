...
<Info>
**Requirements:**
* FEMA Integration Platform version 0.34.5 or higher
...
<Warning>
**Breaking Change:** <br/><br/> If your FEMA Integration Platform instance version is < 0.45.0 and (you are using the connect method from the embed sdk, and need the connection externalId to be returned after the user creates it OR if you want to reconnect a specific connection with an externalId), you must upgrade your instance to >= 0.45.0
</Warning>

- You can use the embedded SDK in your SaaS to allow your users to create connections and store them in FEMA Integration Platform.
...
<Steps>
    ...
    <Step title="Call Connect Method">
        After initializing the SDK, you will have access to a property called `fema` inside your `window` object. Call its `connect` method to open a new connection dialog as follows.

        ```html
        <script> 
        fema.connect({connectorName:'@fema-ipaas/connector-google-sheets'});
        </script>
        ```
        ...
        **Connect Parameters:**
        ...
        | -------------- | -------- | ------ | ------------------------------------------- |
        | connectorName      | ✅      | string | The name of the connector you want to create a connection for.
        ...
        **Connect Result**
        ...
        <Tip>
        You can use the `connections` connector in the builder to retrieve the created connection using its name.
        ![Connections in Builder](/resources/screenshots/connections-connector.png)
        ![Connections in Builder](/resources/screenshots/connections-connector-usage.png)
        </Tip>
    </Step>
</Steps>
