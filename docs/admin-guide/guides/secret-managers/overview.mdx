...
Secret Managers allow you to integrate external secret management systems with FEMA Integration Platform, enabling centralized credential management and enhanced security for your global connections.
...
- **Rotation Support**: Easily rotate credentials without updating workflows
...
Once connected, you can reference secrets from your secret managers when configuring connector connections:

1. Open a connection dialog (either a global connection or one inside the workflow builder)
...
**Workflow builder connections:** Only secret manager connections that are accessible to the current project are shown — this includes platform-scoped connections and project-scoped connections assigned to that project.
...
- When the workflow runs, FEMA Integration Platform authenticates with your secret manager and retrieves the secret
- Secrets are fetched on-demand and never stored in FEMA Integration Platform
- If the secret is updated in your secret manager, workflows will use the new value after the cache expires (up to 1 hour), or immediately after clearing the cache
...
- **Network Security**: Ensure your secret manager is accessible from your FEMA Integration Platform instance
...
- Ensure network connectivity between FEMA Integration Platform and your secret manager
