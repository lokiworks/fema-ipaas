...
export const fileModule: FastifyPluginAsyncZod = async (app) => {
    ...
    systemJobHandlers.registerJobHandler(SystemJobName.FILE_CLEANUP_TRIGGER, async () => {
        await fileService(app.log).deleteStaleBulk([FileType.EXECUTION_LOG, FileType.EXECUTION_LOG_SLICE, FileType.FLOW_STEP_FILE, FileType.TRIGGER_EVENT_FILE, FileType.TRIGGER_PAYLOAD, FileType.WEBHOOK_PAYLOAD])
    })
