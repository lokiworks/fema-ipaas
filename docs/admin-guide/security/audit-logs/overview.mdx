...
FEMA Integration Platform records an audit event for every meaningful action on the platform — a flow gets created, a run finishes, a user signs in, a connection is saved, a signing key is rotated, and so on. Each event captures what happened, who did it, when, and the project or platform it belongs to.
...
### Flow runs
- [Flow run started](/admin-guide/security/audit-logs/execution-started)
- [Flow run finished](/admin-guide/security/audit-logs/execution-finished)
