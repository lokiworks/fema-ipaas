...
A workflow step type (backed by `@fema/connector-agent`) that runs an LLM-driven autonomous loop. Given a prompt, tools, an AI provider/model, and optional structured-output fields, it runs a ReAct-style loop (up to `maxSteps`) where the model can call any configured tool before producing a final answer.

### How it works

- No backend entity of its own — the whole configuration lives inside the workflow version's step settings. The step is a `CONNECTOR` action on `@fema/connector-agent`; `settings.input` holds `agentTools`, `structuredOutput`, `prompt`, `maxSteps`, `aiProviderModel` (`{ provider, model }`), and optional `webSearch`.
