...
#   fema-benchmark:local to benchmark the all-in-one image instead)
...
if [ "${REBUILD_APP:-0}" = "1" ] || ! docker image inspect fema-benchmark:local >/dev/null 2>&1; then
  docker build -t fema-benchmark:local "$ROOT"
else
  echo "Reusing existing fema-benchmark:local (set REBUILD_APP=1 to rebuild)"
fi
...
# (signed with the same FEMA_JWT_SECRET the app validates with) and inject it. Override the secret via env;
# the default matches docker-compose.yml. Short-lived (1 day) — a benchmark run lasts minutes.
JWT_SECRET="${FEMA_JWT_SECRET:-benchmark-secret-key-for-testing}"
export FEMA_WORKER_TOKEN=$(JWT_SECRET="$JWT_SECRET" node -e "const jwt=require('jsonwebtoken'),crypto=require('crypto');process.stdout.write(jwt.sign({id:crypto.randomUUID(),type:'WORKER'},process.env.JWT_SECRET,{expiresIn:'1d',keyid:'1',algorithm:'HS256',issuer:'fema'}))")

echo "=== Starting stack ($WORKER_REPLICAS workers @ 0.5 CPU / 1 GB, concurrency 1) ==="
WORKER_REPLICAS=$WORKER_REPLICAS WORKER_IMAGE=$WORKER_IMAGE FEMA_WORKER_TOKEN=$FEMA_WORKER_TOKEN FEMA_EXECUTION_MODE=SANDBOX_CODE_ONLY $COMPOSE up -d
