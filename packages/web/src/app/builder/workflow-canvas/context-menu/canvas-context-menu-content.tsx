...
export const CanvasContextMenuContent = ({
  ...
  const firstSelectedStep = workflowStructureUtil.getStep(
    selectedNodes[0],
    workflowVersion.trigger,
  );
  ...
  const showPasteAsFirstLoopAction =
    ...
    firstSelectedStep?.type === WorkflowActionType.LOOP_ON_ITEMS &&
    !readonly &&
    contextMenuType === ContextMenuType.STEP;
  const showPasteAsBranchChild =
    selectedNodes.length === 1 &&
    firstSelectedStep?.type === WorkflowActionType.ROUTER &&
    !readonly &&
    contextMenuType === ContextMenuType.STEP;
  const showPasteAsCofBranchChild =
    selectedNodes.length === 1 &&
    (firstSelectedStep?.type === WorkflowActionType.CODE ||
      firstSelectedStep?.type === WorkflowActionType.CONNECTOR) &&
    ...
    <ContextMenuContent>
      ...
      {showCopy && (
        <ContextMenuItem
          disabled={disabled}
          onClick={() => {
            copySelectedNodes({ selectedNodes, workflowVersion });
          }}
        >
