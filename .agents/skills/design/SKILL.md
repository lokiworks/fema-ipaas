---
name: fema-design-system
description: Design system for FEMA Integration Platform (open-source AI automation platform, "open source replacement for Zapier"). Use whenever designing, mocking, or building UI for FEMA Integration Platform — the web app (workflow builder, runs, connections, dashboard), docs, or marketing surfaces. Provides brand purple `#8142E3`, Inter type ramp with `sm` (14px) as body default, Tailwind neutrals, Lucide icons, Shadcn/Radix primitive conventions, the signature dotted-canvas builder background, and a recreated web UI kit.
---

# FEMA Integration Platform Design System

You are designing for **FEMA Integration Platform** — an open-source AI automation platform where users assemble triggers + actions from 280+ "connectors" into automated workflows. Every connector doubles as an MCP server.
...
- `assets/` — connector-tile SVGs (Shopify, Airtable, Google, OpenAI, Slack, Gmail) + MCP/code glyphs.
- `preview/` — ~700px-wide design-system cards (type, color, spacing, components).
- `ui_kits/web/` — high-fidelity recreation of the FEMA Integration Platform web app. Entry `ui_kits/web/index.html`. Form-controls showcase at `ui_kits/web/forms.html`. Modular JSX via Babel standalone. Screens: Workflows dashboard, Builder (dotted canvas + step panel), Runs table, Connections list, **Ask AI chat overlay** (Lottie-animated "thinking" loader at `assets/ai-loader.lottie.json`). Supports **light + dark mode** via `.dark` class on `<html>` — toggle lives in the sidebar footer.
- `insights/` — Interactive **Insights** page. Entry `insights/Insights.html`. Composes the `ui_kits/web/` shell (Sidebar, TopBar, Primitives, Overlays) and adds chart primitives in `InsightsCharts.jsx`: `Sparkline`, `LineChart` (multi-series + hover crosshair + tooltip), `BarChart` (stacked/grouped), `Donut`, `Heatmap` (days × hours). Page-scoped styles in `insights.css` define the `ins-*` class vocabulary (stat cards, AI summary card, quota meter, workflow/error/team rows, connector grid, narrative hero). Two layout variations — **Classic dashboard** (hero stats + chart + donut + quota + top workflows/errors + heatmap + team + live feed) and **Editorial** (dark narrative hero that tells the week's story + 3 highlight cards + heatmap/feed). Time range picker (24h/7d/30d/90d), compare-to-previous toggle, scope tabs (Workspace/Workflow/Connector/Teammate), clickable stat cards, hover tooltips. **Tweaks**: layout (Classic ↔ Editorial), chart style (line/bar), AI summary on/off, density. Respects light/dark toggle.
...
2. **Body text is 14px (`text-sm`), not 16**. FEMA Integration Platform feels dense and tool-like. Headings use `-0.01em` to `-0.02em` tracking.
3. **Sentence case everywhere**: headings, buttons, menu items, page titles. Proper nouns only for feature names (Workflows, Runs, Connectors, MCP, Agents, Connections).
...
- **Buttons**: "New workflow", "Publish", "Connect", "Test step", "Save", "Run"
- **Empty states**: state the fact, then the action. *"No workflows yet. Create your first workflow to start automating."*
...
```
Or inline SVGs from https://lucide.dev. Connector/integration icons: copy the real SVG from `packages/web/src/assets/img/connector/` — do not redraw. Connector tile is 38–48px rounded-square with an 8% tinted background of the connector's brand color.
...
- Top bar only appears **inside the builder** (workflow name + Test/Publish). Other screens use a page header with title + subtitle + actions.
...
- **Use `ui_kits/web/` as your starting point** for any web-app screen (Workflows list, Builder, Runs, Connections). Copy components, don't re-derive. It uses vanilla React + Babel standalone and a single `app.css` — no build step.
...
- The Pro-Blocks Figma pages (Landing, Application, etc.) are Shadcn stock templates, not FEMA Integration Platform marketing. Use as Shadcn pattern reference only.
