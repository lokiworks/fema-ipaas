...
export const streamCsvToSubflows = createAction({
  ...
  aiMetadata: { description: 'Streams a CSV (a URL, an uploaded file, or a file from a previous step) and dispatches one subflow run per batch of rows. Pick it over Call Workflow when the CSV is too large to hold in memory, since it is parsed as a stream and batches are fanned out with bounded concurrency. Rows per batch must be an integer between 1 and 10000 and the target subflow must be published with a "Callable Workflow" trigger; not idempotent, since every batch dispatches a new subflow run.', idempotent: false },
  props: {
    file: Property.File({
    ...
    subflow: subflowDropdown({
      displayName: 'Subflow',
      description:
        'The subflow to call for each batch. Published workflows with a "Callable Workflow" trigger appear here; disabled workflows are marked "(inactive)".',
    }),
