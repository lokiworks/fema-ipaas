...
### Releases & git sync
- **ProjectRelease**: snapshot of flow/table/connection state, applied atomically, `type` GIT_BRANCH/MANUAL/ROLLBACK. Workflow: `releasePlan()` computes a `ProjectSyncPlan` diff (including exact connector version changes), `create()` applies + serializes to a File. Memory lock prevents concurrent releases.
