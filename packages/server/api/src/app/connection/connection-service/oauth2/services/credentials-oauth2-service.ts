...
export const credentialsOauth2Service = (log: FastifyBaseLogger): OAuth2Service<OAuth2ConnectionValueWithApp> => ({
    ...
    async refresh({
        ...
        const { client_id, client_secret } = connection
        const grantType =
        ...
        switch (grantType) {
            case OAuth2GrantType.AUTHORIZATION_CODE: {
                ...
                body.refresh_token = connection.refresh_token
                break
            }
            case OAuth2GrantType.CLIENT_CREDENTIALS: {
                body.grant_type = OAuth2GrantType.CLIENT_CREDENTIALS
                if (connection.scope) {
                    body.scope = resolveValueFromProps(connection.props, connection.scope)
                }
                if (connection.props) {
                    Object.entries(connection.props).forEach(([key, value]) => {
        ...
        const response = (
            await safeHttp.retryingAxios.post(connection.token_url, new URLSearchParams(body), {
                headers,
                timeout: 20000,
            })
        ).data
