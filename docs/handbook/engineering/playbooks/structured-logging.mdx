...
```ts
// ✅ Grouped — clear, and flattens to execution.id / flow.id
logger.info({
...
logger.info({ runId: run.id, executionId: run.id, id: run.id }, 'Flow run started')
```
...
Never a top-level `executionId`, `runId`, or bare `id`. A flow run is `execution: { id }`, a flow is `flow: { id }`. This gives exactly one queryable path per entity.
</Step>
...
| Group | Fields | Entity |
| --- | --- | --- |
| `execution` | `id`, `status`, `environment` | A flow execution |
