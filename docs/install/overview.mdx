...
<Note>
**Going to production?** Pick an install method below to get running, then read [Production Setup](./configure-operate/production-setup) — it's the one opinionated production shape, and every sizing choice flows from a single number.
</Note>
