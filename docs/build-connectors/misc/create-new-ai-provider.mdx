...
ActiveConnectors currently supports the following AI providers:
...
Create a new factory that returns an instance of the `AI` interface in the `packages/connectors/common/src/lib/ai/providers/your-ai-provider.ts` file.
...
Add the new AI provider to the `AiProviders` enum in `packages/connectors/common/src/lib/ai/providers/index.ts` file.

```diff
export const AiProviders = [
+  {
+    logoUrl: 'https://cdn.fema.local/connectors/openai.png',
...
Now we need to tell ActiveConnectors how to authenticate to your AI provider. You can do this by adding an `auth` property to the `AiProvider` object.
...
To test the AI provider, you can use a **universal AI** connector in a workflow. Follow these steps:
...
- Create a workflow that uses our **universal AI** connectors. And select **"Your AI Provider"** as the AI provider in the **Ask AI** action settings.
