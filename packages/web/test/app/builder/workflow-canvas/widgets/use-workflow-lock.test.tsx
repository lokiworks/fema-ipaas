 ...
 *
 * useWorkflowLock decides how the builder refreshes after a lock take-over: when a
 * run is being viewed it navigates client-side to /workflows/:id (embed-safe),
...
import { useWorkflowLock } from '@/app/builder/workflow-canvas/widgets/use-workflow-lock';
...
vi.mock('@/app/builder/workflow-canvas/hooks', () => ({
  workflowCanvasHooks: {
...
describe('useWorkflowLock take-over branch', () => {
  ...
  it('navigates to the workflow client-side when a run is being viewed', async () => {
    ...
    expect(harness.navigate).toHaveBeenCalledWith(`/workflows/${WORKFLOW_ID}`);
