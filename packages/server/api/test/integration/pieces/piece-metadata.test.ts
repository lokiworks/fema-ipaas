...
import { DefaultProjectRole, FlowTriggerType, PackageType, PieceType, PrincipalType } from '@fema/shared'
...
describe('Piece Metadata CE API', () => {
    ...
    describe('GET /v1/pieces/:scope/:name', () => {
        it('should get piece by scope and name', async () => {
            ...
            const response = await ctx.get(`/v1/pieces/@fema/ce-scoped-piece?projectId=${ctx.project.id}`)

            expect(response?.statusCode).toBe(StatusCodes.OK)
            const body = response?.json()
            expect(body.name).toBe('@fema/ce-scoped-piece')
        })
    })
    ...
    describe('release-compatibility fallback', () => {
        it('GET /v1/pieces/:scope/:name falls back to the newest compatible version when latest requires a newer release', async () => {
            ...
            const ctx = await createTestContext(app!)
            const response = await ctx.get('/v1/pieces/@fema/connector-release-test')
        ...
        it('GET /v1/pieces/:scope/:name returns 404 when all versions are incompatible', async () => {
            ...
            const ctx = await createTestContext(app!)
            const response = await ctx.get('/v1/pieces/@fema/connector-all-incompatible')
    ...
    describe('audience filtering (canvas filter)', () => {
        ...
        it('GET /v1/pieces/:scope/:name hides audience:ai by default', async () => {
            const ctx = await createTestContext(app!)
            const mockPiece = createMockPieceMetadata({
                name: '@fema/audience-scoped-piece',
                pieceType: PieceType.OFFICIAL,
                packageType: PackageType.REGISTRY,
                actions: buildActions(),
            })
            ...
            const response = await ctx.get('/v1/pieces/@fema/audience-scoped-piece')
