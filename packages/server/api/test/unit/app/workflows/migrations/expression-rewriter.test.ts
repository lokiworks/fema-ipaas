...
import { expressionRewriter } from '../../../../../src/app/workflows/workflow-version/migrations/expression-rewriter'
...
describe('expressionRewriter.rewriteStepReferences', () => {
    ...
    describe('P. Unparseable token — quote-blanking recovery', () => {
        it('P1. unterminated string in bracket key (the Cardis workflow case)', () => {
            expect(rewrite('{{step_15[\'body\'][\'url]}}', ['step_15']))
                .toBe('{{step_15[\'output\'][\'body\'][\'url]}}')
        })
        it('P2. canonical step ref recovered without a workflow step list', () => {
            expect(rewrite('{{step_3[\'body\'][\'url]}}'))
                .toBe('{{step_3[\'output\'][\'body\'][\'url]}}')
        })
        it('P3. custom step name recovered from the workflow step list', () => {
    ...
    // Regression: a naive /\{\{(.*?)\}\}/ tokenizer stops at the first `}}`, which
    // can land inside the expression (string literals, nested objects). That truncates
    // the token, acorn fails to parse it, and the ref is left un-rewritten while the
    // resolver still evaluates it whole — silently breaking the reference. The shared
