...
Flow code (Code steps and connector actions) always runs inside a **sandbox** that wraps the engine process. `FEMA_EXECUTION_MODE` decides which sandbox. It is the most important security choice in a self-hosted deployment: it decides whether a malicious flow is contained to one worker pod or can reach the kernel.
