...
description: "Connect HashiCorp Vault to FEMA Integration Platform for enterprise-grade secret management"
---
...
Before connecting HashiCorp Vault to FEMA Integration Platform, ensure you have:
...
Once the connection is saved, you can reference Vault secrets inside any connector connection dialog — in global connections (Platform Admin) or directly in the workflow builder.
...
The connection will automatically retrieve the secret from Vault when the workflow runs.

<Warning>
  If you update a secret in Vault and the change isn't reflected in your workflows, the cached value may still be active. Use the **refresh icon** next to the connection in the Secret Managers page to clear its cache immediately, or wait up to 1 hour for it to expire automatically. See [Caching](/admin-guide/guides/secret-managers/overview#caching) for details.
</Warning>
