...
describe('connector child exit classification', () => {
    it.each([
        ...
        expect(error).toMatchObject({ name: 'ConnectorMemoryLimitError', type: ExecutionErrorType.USER })
        expect(JSON.parse(error.message).message).toBe('The connector ran out of memory')
    })
    ...
    it('reports any other abnormal exit as an engine error carrying the child output', () => {
        ...
        expect(error).toMatchObject({ name: 'ConnectorProcessExitedError', type: ExecutionErrorType.ENGINE })
