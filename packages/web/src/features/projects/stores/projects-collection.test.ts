...
import type { ProjectWithLimits } from '@fema-ipaas/shared';
import {
  and,
  createCollection,
...
describe('useAll filter', () => {
  ...
  it('shows TEAM projects and only own PERSONAL from a full tenant collection', () => {
    const projects = [
      makeProject('teamA', ProjectType.TEAM, 'tenantOwner'),
      makeProject('teamB', ProjectType.TEAM, 'tenantOwner'),
...
describe('useAllTenantProjects filter', () => {
  const allProjects = [
  ...
  it('returns all projects when both types are specified', () => {
    expect(
      query(allProjects, {
        type: [ProjectType.TEAM, ProjectType.PERSONAL],
      }),
    ).toHaveLength(4);
  });
});
...
describe('setCurrentProject', () => {
  ...
  it('replaces the project ID segment in the pathname and navigates', () => {
    setCurrentProject('projNew', '/projects/projOld/workflows');
    expect(window.location.href).toBe('/projects/projNew/workflows');
  });
  ...
  it('does not modify paths without a /projects/:id segment', () => {
    setCurrentProject('projNew', '/tenant/projects');
    expect(window.location.href).toBe('/tenant/projects');
  });
});
