...
import { UpgradeConnectorVersionContent } from './upgrade-connector-version-dialog';

type DialogView = 'upgrade' | 'advanced';

const UpdateConnectorVersionDialog: React.FC<
  UpdateConnectorVersionDialogProps
> = ({ step, currentVersion }) => {
  const [view, setView] = useState<DialogView | null>(null);
  const connectorName = step.settings.connectorName;
  const { connectorVersions, isLoading } =
    connectorsHooks.useConnectorVersions(connectorName);
          ...
          <DialogHeader className="mb-0">
            <DialogTitle>
              {view === 'upgrade'
                ...
                : t('Update Connector Version')}
            </DialogTitle>
          </DialogHeader>
          {view === 'upgrade' && latestVersion && (
            <UpgradeConnectorVersionContent
              key="upgrade"
              step={step}
...
export { UpdateConnectorVersionDialog };

type UpdateConnectorVersionDialogProps = {
...
const AdvancedForm: React.FC<AdvancedFormProps> = ({
  ...
  const patchVersions = (connectorVersions ?? []).filter((p) => {
