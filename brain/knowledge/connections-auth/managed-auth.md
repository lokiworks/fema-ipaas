...
- **v1/v2** (no `version` field): legacy nested `connectors` object.
- **v3** (`version: "v3"`): flat `connectorsFilterType` + `connectorsTags`.
- **v4** (`version: "v4"`): `connectorSet` **required**, is the connector set's **key**.

### Gotchas
- Endpoint is public (`securityAccess.public()`) — the JWT signature is the security.
- Connector-set assignment falls back: explicit `connectorSet` key (v4) → first legacy `connectorsTags` entry matched against set `key` → platform Default set (with a warn log if no match). The project plan is not written.
