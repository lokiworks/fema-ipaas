...
export const ApRouterStartCanvasEdge = ({
  ...
  const generateAlignedLayoutPath = () => {
    ...
    let path = `M ${layoutTarget.x} ${
      layoutTarget.y - workflowCanvasConsts.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE
    }`;
    ...
    if (distanceBetweenSourceAndTarget >= workflowCanvasConsts.ARC_LENGTH) {
      ...
      if (data.drawHorizontalLine) {
        ...
        path +=
          layoutSource.x > layoutTarget.x
            ? workflowCanvasConsts.ARC_LEFT_UP
            : workflowCanvasConsts.ARC_RIGHT_UP;
      }
  ...
  const buttonPosition = isHorizontal
    ? {
        ...
        x:
          ...
          workflowCanvasConsts.FEMA_NODE_SIZE.ADD_BUTTON.width,
        y: targetY - workflowCanvasConsts.FEMA_NODE_SIZE.ADD_BUTTON.height / 2,
      }
    : {
        x: targetX - workflowCanvasConsts.FEMA_NODE_SIZE.ADD_BUTTON.width / 2,
        y: targetY - verticalLineLength / 2,
      };

  const labelBoxWidth = workflowCanvasConsts.FEMA_NODE_SIZE.STEP.width - 10;
  const labelBoxHeight =
    workflowCanvasConsts.LABEL_HEIGHT +
    workflowCanvasConsts.LABEL_VERTICAL_PADDING;
  const getLabelBoxPosition = () => {
    ...
    return {
      ...
      y:
        ...
        workflowCanvasConsts.FEMA_NODE_SIZE.ADD_BUTTON.height -
        30,
    };
  };
