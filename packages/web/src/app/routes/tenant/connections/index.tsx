...
export default function TenantConnectionsPage() {
  ...
  const filters: DataTableFilters<
    keyof TenantConnectionsListItem | 'ownerIds'
  >[] = [
    {
      type: 'input',
      title: t('Name'),
  ...
  const columns: ColumnDef<RowDataWithActions<TenantConnectionsListItem>>[] = [
    {
      accessorKey: 'displayName',
      size: 280,
      header: ({ column }) => (
  ...
  return (
    <div className="flex flex-col w-full">
      <DashboardPageHeader
        title={t('Connections')}
        description={t(
          'All app connections across every workspace on this tenant',
        )}
      />
      {owners?.truncated && (
        <div className="px-6 pb-2 text-xs text-muted-foreground">
          {t('Owner filter is limited to the first {count} owners', {
            count: MAX_TENANT_CONNECTION_OWNERS,
          })}
        </div>
      )}
      <DataTable
        ...
        emptyStateTextDescription={t(
          'Connections created in any workspace on this tenant will appear here.',
        )}
...
const ScopeBadge = ({ scope }: { scope: ConnectionScope }) => {
  if (scope === ConnectionScope.TENANT) {
    return (
...
const WorkspacesCell = ({
  ...
  workspaces: TenantConnectionsListItem['workspaces'];
}) => {
