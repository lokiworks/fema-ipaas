...
describe('formatConnectorError', () => {
    it('extracts status, request, response, and apiMessage from a Jira-style 403 HttpError', () => {
        ...
        const result = formatConnectorError(error)
...
<Error><Code>NoSuchKey</Code><Message>The specified key does not exist.</Message></Error>`
    ...
    it('returns a default message for nil input', () => {
        expect(formatConnectorError(null).message).toBe('Unknown error')
        expect(formatConnectorError(undefined).message).toBe('Unknown error')
    })
    ...
    it('treats a string input as the message, stripping any stack frames', () => {
        ...
        const result = formatConnectorError(raw)

        expect(result.message).toBe('HttpError: something went wrong')
        expect(result.message).not.toContain('at ')
    })

    it('returns the input verbatim if it is already a FriendlyConnectorError', () => {
        const original = formatConnectorError(new TestHttpError({ status: 404, body: { message: 'not found' } }, { body: {} }))
        const second = formatConnectorError(original)
        expect(second).toBe(original)
    })

    it('fills in raw on an already-formatted error that has none', () => {
        const original = formatConnectorError(new Error('Boom'))
        expect(original.raw).toBeUndefined()

        const second = formatConnectorError(original, { raw: 'Error: Boom\n    at x' })
    ...
    it('does not overwrite an existing raw on an already-formatted error', () => {
        const original = formatConnectorError(new Error('Boom'), { raw: 'original raw' })

        const second = formatConnectorError(original, { raw: 'new raw' })
    ...
    it('attaches the raw dump when provided in options', () => {
        ...
        const result = formatConnectorError(error, { raw: 'Error: Boom\n    at somewhere' })
    ...
    it('does not set raw when no raw option is provided', () => {
        const result = formatConnectorError(new Error('Boom'))

        expect(result.raw).toBeUndefined()
    })

    it('truncates a very long raw dump', () => {
        const result = formatConnectorError(new Error('Boom'), { raw: 'y'.repeat(20000) })
...
describe('tryParseFriendlyConnectorError', () => {
    it('parses a JSON-stringified FriendlyConnectorError', () => {
        const original = formatConnectorError(new TestHttpError({ status: 429, body: { message: 'Too many requests' } }, { body: {} }))
        const json = JSON.stringify(original)
        const parsed = tryParseFriendlyConnectorError(json)
        expect(parsed).not.toBeNull()
        expect(parsed?.status).toBe(429)
        expect(parsed?.apiMessage).toBe('Too many requests')
    })

    it('returns the value if already a FriendlyConnectorError object', () => {
        const original = formatConnectorError(new Error('x'))
        expect(tryParseFriendlyConnectorError(original)).toEqual(original)
    })

    it('returns null for non-structured strings', () => {
        expect(tryParseFriendlyConnectorError('not a friendly error')).toBeNull()
        expect(tryParseFriendlyConnectorError('{"foo":"bar"}')).toBeNull()
    })

    it('returns null for nil values', () => {
        expect(tryParseFriendlyConnectorError(null)).toBeNull()
        expect(tryParseFriendlyConnectorError(undefined)).toBeNull()
    })
})
