...
describe('recordTriggerRun', () => {
    it('maps OK to COMPLETED', async () => {
        ...
        await recordTriggerRun({ apiClient, log, flowVersion: buildPieceFlowVersion('@fema/connector-slack'), platformId: 'p1', status: EngineResponseStatus.OK })

        expect(recordTriggerRunRpc).toHaveBeenCalledWith({ platformId: 'p1', pieceName: '@fema/connector-slack', status: TriggerRunStatus.COMPLETED })
    })
    ...
    it('maps non-OK statuses to FAILED', async () => {
        ...
        await recordTriggerRun({ apiClient, log, flowVersion: buildPieceFlowVersion('@fema/connector-slack'), platformId: 'p1', status: EngineResponseStatus.INTERNAL_ERROR })

        expect(recordTriggerRunRpc).toHaveBeenCalledWith({ platformId: 'p1', pieceName: '@fema/connector-slack', status: TriggerRunStatus.FAILED })
    })

    it('skips non-piece triggers', async () => {
        const recordTriggerRunRpc = vi.fn(async () => undefined)
        const apiClient = { recordTriggerRun: recordTriggerRunRpc } as unknown as WorkerToApiContract
        const emptyTriggerFlowVersion = { ...buildPieceFlowVersion('@fema/connector-slack'), trigger: { type: FlowTriggerType.EMPTY, settings: {} } } as unknown as FlowVersion
    ...
    it('never throws when the rpc fails', async () => {
        ...
        await expect(recordTriggerRun({ apiClient, log, flowVersion: buildPieceFlowVersion('@fema/connector-slack'), platformId: 'p1', status: EngineResponseStatus.OK })).resolves.toBeUndefined()
    })
})
