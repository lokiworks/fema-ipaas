...
|---|---|---|---|
| `combine` | `combine(text1; text2; separator)` | Joins two or more values into one connector of text. | `combine(John; Smith; " ")` → `John Smith` |
...
| `now` | `now()` | Returns the current date and time at the moment the workflow runs. | `now()` → `2025-01-15T14:30:00Z` |
| `today` | `today()` | Returns today's date with no time attached. | `today()` → `2025-01-15` |
| `to_date` | `to_date(text)` | Turns a text value into a proper date the workflow can work with. | `to_date(January 15, 2025)` → `2025-01-15T00:00:00Z` |
...
| `join_list` | `join_list(list; separator)` | Turns a list into a single connector of text with a separator. | `join_list([bug;urgent;backend]; ", ")` → `bug, urgent, backend` |
...
No functions are currently deprecated. When a function gets replaced or scheduled for removal, it will appear here with the recommended replacement and the version it will be removed in. Saved workflows that use a deprecated function keep running — the editor just shows a strikethrough badge to flag the issue.
