...
How workflows are authored, triggered, executed, and organized in FEMA Integration Platform. Skim map of the core automation domain.

### Workflows
Versioned directed graph (trigger + actions) stored as JSONB. All 26 modification types go through ONE endpoint: `POST /v1/workflows/:id` with a `WorkflowOperationRequest` discriminated union.
- Entities: `workflow` (status, folderId, publishedVersionId, externalId, createdBy) + `workflow_version` (immutable once LOCKED; DRAFT is the editable copy). Current schemaVersion `'22'`.
...
- Frontend builder = XYWorkflow canvas + Zustand state slices. Supports vertical/horizontal layout and PNG export.
- Gotcha: `createdBy` (MCP/AGENT) drives the "AI" badge; distinct from `ownerId`.

### Workflow Runs
One execution instance per workflow version, trigger → terminal state. 12 statuses (3 non-terminal: QUEUED/RUNNING/PAUSED; 9 terminal incl. FAILED/TIMEOUT/QUOTA_EXCEEDED/MEMORY_LIMIT_EXCEEDED).
...
### Triggers
Defines how/when a workflow starts. Registered as a `TriggerSource` (unique per projectId/workflowId/simulate); dedup state in Redis.
...
### Webhooks
Primary entry point for inbound HTTP → workflow execution. 5 public routes: sync/async × prod/draft + test-only.
- Sync (`/:workflowId/sync`) blocks the connection and returns the workflow response via `engineResponseWatcher` (default 30s timeout). Async returns 200 + `x-webhook-id` and queues a BullMQ job.
...
- Only platform owners manage CUSTOM templates; OFFICIAL/SHARED can't be edited/deleted. Workflow validation + connector extraction run before save.

## Pages

- **Workflows** — the versioned trigger + action graph, DRAFT/LOCKED, publishing
- **Workflow Runs** — the status state machine and RunTimeline phases
- **Action Runs** — a single step executed outside any workflow, synchronously
- **Triggers** — POLLING / WEBHOOK / APP_WEBHOOK / MANUAL
- **Human Input** — forms, approvals, the resume confirmation page
- **Subflows** — workflow-calls-workflow: Callable Workflow, Call Workflow, streaming fan-out
- **Folders** — workflow organization; the uncategorized sentinel
