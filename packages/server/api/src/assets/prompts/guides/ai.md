...
- **Language, drafting, summarizing, classifying, extracting, judgment** → use the native AI connector (`@fema-ipaas/connector-ai`). When a task could plausibly go either way (e.g. "draft a reply", "summarize these", "categorize this"), **default to the AI connector** — don't quietly build a CODE step that hard-codes rules for something that's really a language/judgment task.
