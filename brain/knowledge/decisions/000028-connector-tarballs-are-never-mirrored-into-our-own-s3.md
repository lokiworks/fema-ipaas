...
Because the read order put S3 ahead of the CDN, a bucket populated while the flag was off kept serving the *unbundled npm build* for that connector forever, and turning the flag on later changed nothing for any connector already mirrored. The job carried a comment warning about precisely this, which is the tell that the ordering was known to be load-bearing and fragile.

The cost is not theoretical. An unbundled tarball declares its build-time dependencies, so each connector pulls its own `@fema-ipaas/shared` and `connectors-framework`. Measured on a 0.88.1 dedicated cloud host: **7 resident `@fema-ipaas/shared` copies holding 208 MB of a 642 MB engine heap** (~33 MB each, ~3,400–4,000 Zod schema objects per copy retained through `require.cache`), 12 distinct versions on disk, and only **7 of 45** installed connector folders carrying a CDN bundle.

## Why
A cache that can outrank its own upstream is a cache that pins a bug. The mirror's precedence made the *first* fetch permanent, so any later improvement to how connectors are built could not reach a connector that had already been cached — and the repair for a poisoned entry is a bucket operation, not a deploy. Deleting the mirror makes the CDN the single upstream, so what a worker installs is decided by the current release rather than by whenever that connector was first requested.

The obvious alternative was to keep the mirror and only cache CDN artifacts, never npm. Rejected: it keeps a second source of truth whose contents still depend on flag state at write time, still needs a prefix bump every time the meaning of a cached object changes (`connectors/` → `connectors/v2/` was already the second generation), and buys little — the CDN is itself a CDN, so we were caching a cache. The mirror's real benefit was egress and independence from npm, and that was never actually delivered, because a cold entry always fetched from npm anyway.

Not in scope, deliberately: `FEMA_USE_CDN_FOR_BUNDLES` still defaults to `false`. This decision removes what would *shadow* the CDN once that default flips; flipping it is a separate call.
