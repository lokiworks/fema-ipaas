...
vi.mock(
  '@/app/builder/connector-properties/dynamic-connector-properties-error-boundary',
  () => ({
...
vi.mock('@/app/builder/connector-properties/generic-properties-form', () => ({
...
type MutateOptions = {
  onSuccess: (response: { options: ConnectorPropertyMap }) => void;
  onError: () => void;
};
...
vi.mock('@/features/connectors', async () => {
  const { formUtils } = await vi.importActual<
    typeof import('@/features/connectors/utils/form-utils')
  >('@/features/connectors/utils/form-utils');
  return {
    formUtils,
    connectorsHooks: {
      useConnectorOptions: () => ({
...
import { DynamicProperties } from '@/app/builder/connector-properties/dynamic-connector-property';

const shortTextSchema = (fieldNames: string[]): ConnectorPropertyMap => {
  const schema: ConnectorPropertyMap = {};
...
describe('DynamicProperties refresher change', () => {
  ...
  const resolveOptions = (options: ConnectorPropertyMap) => {
