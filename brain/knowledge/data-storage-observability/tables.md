...
A built-in relational database inside FEMA Integration Platform: users store structured data (typed columns, rows) without an external DB, edited in a spreadsheet-like UI and wired into workflows. Available in CE, EE, and Cloud.
...
- **TableWebhook**: links a table event to a workflow. Events: `RECORD_CREATED`, `RECORD_UPDATED`, `RECORD_DELETED`.
- Services: `table.service.ts`, `field.service.ts`, `record.service.ts`, `record-side-effects.ts`.

### How it works
- After record create/update/delete, `recordSideEffects.handleRecordsEvent()` finds matching TableWebhooks and triggers their linked workflows with the record as payload.
- The **Tables connector** (`packages/connectors/core/tables/`) gives workflows triggers (New/Updated/Deleted Record) and actions (Create/Get/Find/Update/Delete Record, Clear Table), calling the internal API with a Bearer token.
...
- `packages/server/api/src/app/tables/record/` — record service (CRUD, bulk ops), controller, Record and Cell entities, record side effects that fire TableWebhook workflows
...
- `packages/connectors/core/tables/` — the Tables connector: triggers and actions workflows use to read and write tables
