...
function buildClassicSchema(
  ...args: Parameters<typeof connectorPropertiesUtils.buildSchema>
): ZodType {
  const miniSchema = connectorPropertiesUtils.buildSchema(...args);
  ...
  switch (type) {
    case FlowActionType.CONNECTOR: {
      if (
        ...
        connector.actions[actionNameOrTriggerName]
      ) {
        return buildClassicSchema(
          connector.actions[actionNameOrTriggerName].props,
          connector.auth,
          connector.actions[actionNameOrTriggerName].requireAuth,
        );
      }
      return z.object({});
    }
    case FlowTriggerType.CONNECTOR: {
      if (
        connector &&
        actionNameOrTriggerName &&
        connector.triggers[actionNameOrTriggerName]
      ) {
        return buildClassicSchema(
          connector.triggers[actionNameOrTriggerName].props,
          connector.auth,
          connector.triggers[actionNameOrTriggerName].requireAuth,
        );
      }
      return z.object({});
    }
...
function buildCustomAuthValueSchema(auth: ConnectorAuthProperty) {
...
function buildOIDCValueSchema(auth: ConnectorAuthProperty) {
...
function buildConnectionSchema(
  auth: ConnectorAuthProperty | null,
  options: {
    isGlobalConnection: boolean;
    showConnectionNameField: boolean;
  },
