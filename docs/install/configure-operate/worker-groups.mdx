...
Once a project is assigned to a group, its flow and webhook runs are routed to that group's dedicated queue (`project-<group-name>-jobs`) and picked up only by workers carrying the matching `FEMA_WORKER_GROUP_ID` with `FEMA_PROJECT_WORKER=true`. Projects without a group keep drawing from the shared queue.
