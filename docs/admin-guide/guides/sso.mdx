...
description: "Configure Single Sign-On (SSO) to enable secure, centralized authentication for your FEMA Integration Platform platform"
icon: 'key'
---
...
Single Sign-On (SSO) allows your team to authenticate using your organization's existing identity provider, eliminating the need for separate FEMA Integration Platform credentials. This improves security, simplifies user management, and provides a seamless login experience.

## Prerequisites

Before configuring SSO, ensure you have:

- **Admin access** to your FEMA Integration Platform platform
- **Admin access** to your identity provider (Google, GitHub, Okta, or JumpCloud)
- The **redirect URL** from your FEMA Integration Platform SSO configuration screen

## Accessing SSO Configuration

Navigate to **Platform Settings** → **SSO** in your FEMA Integration Platform admin dashboard to access the SSO configuration screen.
...
FEMA Integration Platform supports multiple SSO providers to integrate with your existing identity management system.
...
<Steps>
    ...
    <Step title="Configure Redirect URI">
        Copy the **Redirect URL** from the FEMA Integration Platform SSO configuration screen and add it to the **Authorized redirect URIs** in Google Cloud Console.
    </Step>
    <Step title="Copy Credentials to FEMA Integration Platform">
        Copy the **Client ID** and **Client Secret** from Google and paste them into the corresponding fields in FEMA Integration Platform.
    </Step>
...
<Steps>
    ...
    <Step title="Register New Application">
        ...
        - **Application name**: Choose a recognizable name (e.g., "FEMA Integration Platform SSO")
        - **Homepage URL**: Enter your FEMA Integration Platform instance URL
    </Step>
    <Step title="Configure Authorization Callback">
        Copy the **Redirect URL** from the FEMA Integration Platform SSO configuration screen and paste it into the **Authorization callback URL** field.
    </Step>
    ...
    <Step title="Copy Credentials to FEMA Integration Platform">
        Copy the **Client ID** and **Client Secret** and paste them into the corresponding fields in FEMA Integration Platform.
    </Step>
    <Step title="Save Configuration">
        Click **Finish** to complete the setup.
    </Step>
...
<Steps>
    ...
    <Step title="Configure General Settings">
        Enter an **App name** (e.g., "FEMA Integration Platform") and optionally upload a logo. Click **Next**.
    </Step>
    <Step title="Configure SAML Settings">
        - **Single sign-on URL**: Copy the SSO URL from the FEMA Integration Platform configuration screen
        - **Audience URI (SP Entity ID)**: Enter `FEMA Integration Platform`
    ...
    <Step title="Configure FEMA Integration Platform">
...
<Steps>
    <Step title="Create an Enterprise Application">
        ...
        Name it (e.g., "FEMA Integration Platform") and select **Integrate any other application you don't find in the gallery (Non-gallery)**.
    </Step>
    ...
    <Step title="Set Identifier and Reply URL">
        ...
        - **Identifier (Entity ID)**: `FEMA Integration Platform`
        - **Reply URL (Assertion Consumer Service URL)**: paste the SSO URL from the FEMA Integration Platform configuration screen
    </Step>
    ...
    <Step title="Copy the Federation Metadata">
        ...
        You can paste this URL directly into the **IdP Metadata** field in FEMA Integration Platform — FEMA Integration Platform will fetch the metadata XML automatically. Alternatively, open the URL in a browser, save the XML, and paste its contents.
    </Step>
    <Step title="Copy the Signing Certificate">
        Download the **Certificate (Base64)** from the **SAML Certificates** section. Open the file and copy its contents (including the `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` markers) into the **Signing Key** field in FEMA Integration Platform.
    </Step>
    <Step title="Assign Users">
        Go to **Users and groups** in the application and assign the users or groups that should be allowed to sign in.
    </Step>
    <Step title="Save Configuration">
        Click **Save** in FEMA Integration Platform to complete the setup.
    </Step>
</Steps>
...
<Steps>
    ...
    <Step title="Configure ACS URL">
        Copy the **ACS URL** from the FEMA Integration Platform configuration screen and paste it into the **ACS URLs** field in JumpCloud.
        
        ![JumpCloud ACS URL](/resources/screenshots/jumpcloud/acl-url.png)
    </Step>
    <Step title="Configure SP Entity ID">
        Set the **SP Entity ID** (Audience URI) to `FEMA Integration Platform`.
    </Step>
    ...
    <Step title="Configure IdP Metadata in FEMA Integration Platform">
        Paste the exported metadata XML into the **IdP Metadata** field in FEMA Integration Platform.
    </Step>
...
<AccordionGroup>
    ...
    <Accordion title="SAML authentication fails">
        ...
        - If you pasted a metadata URL, make sure it is publicly reachable (FEMA Integration Platform fetches it server-side)
...
If you encounter issues during SSO setup, please contact our enterprise support or [sales team](https://github.com/lokiworks/fema-ipaas/sales).
