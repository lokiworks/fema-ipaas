...
The step's inputs and its output shape are unchanged, so anything reading the step's result or its structured output keeps working. Sub-flow, MCP server, knowledge base and connector action tools all run on the new path.
...
- Only if a flow uses FEMA Integration Platform-provided AI on a model outside the Fast/Expert/Heavy tiers. Choose a tier the next time you edit that step.
...
Log fields are now grouped under the entity they belong to, so each thing has one name. A flow run id is `execution.id` (it used to appear as `runId`, `executionId`, or a bare `id`), and other ids follow the same shape (`flow.id`, `project.id`, `job.id`, …). Errors are logged under `error`. This only changes how logs look — nothing about the API, database, or flows changes.
...
This is an important behavior change in how flow run logs are stored and capped, but **no action is required** — the existing default (`FEMA_MAX_EXECUTION_LOG_SIZE_MB=50`) is preserved.
...
- See [Limits → Files & flow run logs](/install/reference/limits#files-execution-logs) for the full behavior, environment variables, and defaults.
...
- The `LOCK_AND_PUBLISH` operation no longer resolves connector versions at publish time — steps run with the exact version stored in the flow.
...
### Do you need to take action?
- **Pinned connector versions** — if you create or update flows via the REST API with wildcard versions (`~` or `^`), switch to exact versions; wildcards are silently stripped. If your publish flow relied on `LOCK_AND_PUBLISH` resolving wildcards, set the exact version on each step before publishing. Use `GET /v1/connectors/:name/versions` to list available versions.
...
- If you're using the Todos feature, update your flows to use the new approvals channels available from the approvals tab in the connector selector.
...
- Log size configuration has changed: `FEMA_MAX_FILE_SIZE_MB` no longer controls flow run logs. Use `FEMA_MAX_EXECUTION_LOG_SIZE_MB` instead.
...
### What has changed?
- `FEMA_FLOW_WORKER_CONCURRENCY` and `FEMA_SCHEDULED_WORKER_CONCURRENCY` are now deprecated all jobs have single queue and replaced with `FEMA_WORKER_CONCURRENCY`

### When is action necessary?

- If you are using `FEMA_FLOW_WORKER_CONCURRENCY` or `FEMA_SCHEDULED_WORKER_CONCURRENCY`, you should replace them with `FEMA_WORKER_CONCURRENCY`
...
- When editing existing flows that use the "Dynamic Value" toggle on "Array of Properties" inputs (such as the "files" parameter in the "Extract Structured Data" action of the "Utility AI" connector), the end user will need to remap the values again.
...
- `FEMA_SANDBOX_RUN_TIME_SECONDS` is now deprecated and replaced with `FEMA_FLOW_TIMEOUT_SECONDS`
...
- If you are using `FEMA_SANDBOX_RUN_TIME_SECONDS` to set the sandbox run time limit, you should switch to `FEMA_FLOW_TIMEOUT_SECONDS`
