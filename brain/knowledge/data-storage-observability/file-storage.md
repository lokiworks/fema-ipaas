...
- **Expiring execution files** (`EXECUTION_LOG`, `WORKFLOW_STEP_FILE`, `TRIGGER_EVENT_FILE`, `WEBHOOK_PAYLOAD`, deprecated `TRIGGER_PAYLOAD`) → configurable via `FILE_STORAGE_LOCATION`.
- **Non-expiring files** (platform assets, avatars, knowledge base, project releases, sample data) → always DB.
- Exception: `WORKFLOW_BUNDLE` never expires but uses `FILE_STORAGE_LOCATION` so workers can fetch it from S3 via signed URLs.
...
- `httpClient` **skips retries entirely for stream bodies** (`isStream ? 0 : retries`) — the retry loop reuses the pre-serialized body, so replaying a drained `Readable`/form-data `PassThrough` would send a truncated body. Applies to every connector sending a stream through `httpClient`, not just file actions.
- User-facing docs: [Large File Streaming](../../../docs/build-connectors/connector-reference/large-file-streaming.mdx).

### Gotchas
- **On cloud the real key prefix is doubled — `<bucket>/<bucket>/…` — so ad-hoc CLI work against the bucket silently finds nothing.** Cloud's `FEMA_S3_ENDPOINT` embeds the bucket as a *path segment* (`https://<account>.r2.cloudflarestorage.com/ap-files-prod`), and `getS3Client` sets `forcePathStyle: true` whenever an endpoint is present, so the SDK appends the bucket again. An object the app stores as `connectors/x.tgz` actually lands at `ap-files-prod/connectors/x.tgz` inside bucket `ap-files-prod`. Symptom when you get it wrong: `aws s3 ls` returns `NoSuchKey` on a *prefix* listing (Aug 2026 — cost three attempts to spot). Strip the trailing `/<bucket>` from the endpoint for the CLI, then prepend the bucket name to the prefix; or better, do bulk work through `s3Helper` so the same client resolves the same paths. Note cloud's object store is **Cloudflare R2** while the connector CDN is a **DigitalOcean Space** — two different systems, easy to conflate.
- **The live connector-bundle cache sits *inside* the legacy one — `connectors/v2/` is nested under `connectors/`, so a recursive delete of `connectors/` takes the active cache with it.** `S3_CONNECTORS_PREFIX` in `connector-bundle.ts` is `connectors/v2/`; the bare `connectors/` keys beside it are pre-CDN tarballs left by the older writer. Combined with the doubled prefix above, the real keys are `ap-files-prod/connectors/…` (legacy) and `ap-files-prod/connectors/v2/…` (live). Probe both with `wrangler r2 object get` before any prefix-wide operation — wiping v2 used to be survivable because it refilled lazily, at the cost of a burst of cache misses on every connector. **Both prefixes are now dead storage and safe to sweep:** the `BUNDLE_CONNECTOR` job and the S3 mirror were removed, so `resolve()` no longer reads or writes either prefix and registry connectors redirect straight to the CDN (else npm). The mirror was deleted because it was written from whichever source was preferred *at cache time* and then took precedence over the CDN forever — a bucket populated before the CDN became preferred kept serving the unbundled npm build, which is what fans out one `@fema-ipaas/shared` copy per connector in the engine (see the Workers page).
...
- `packages/server/engine/src/lib/connector-context/file-uploader.ts` — backs `ctx.files.write()` for connectors
