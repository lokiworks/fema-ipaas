...
<Note>
**Scope:** the recommended production setup (`SANDBOX_CODE_ONLY`, `FEMA_REUSE_SANDBOX=true`, one flow per worker), which self-hosted deployments and **dedicated Cloud workers** run. The shared Cloud freemium pool uses a different sandbox. See [Production Setup](/install/configure-operate/production-setup).
</Note>
...
**Cold** means a worker that has never seen this flow: it installs the connectors, fetches the bundle, and forks an engine first. **Warm** reuses all three. You hit cold when:
...
<img src="/resources/diagrams/run-timeline-breakdown.png" alt="Run latency breakdown popover: a stacked bar split into Queue (20 ms, waiting for a free worker), Setup (19 ms, installing connectors and engine), Warm-up (0 ms, starting the engine), and Run (32 ms, executing your flow)" />
...
| **Setup** | Fetching the flow bundle and installing the connectors, engine, and code steps into the sandbox (provisioning). | ~0 (cached) | ~0.5 s |
...
For a flow that does real work — a step calling a third-party API — that outbound call dominates **Run**, and the FEMA Integration Platform overhead is the smaller number next to it.
...
<Note>
The same setup measures ~165 ms unloaded and ~200 ms average under sustained peak (160 workers, p50 180 ms / p99 652 ms): the shared database absorbing load, not the flow slowing. Always ask which load a figure was measured under. Throughput view: [Benchmark](/install/architecture/benchmark).
</Note>
