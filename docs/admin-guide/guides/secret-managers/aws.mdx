...
description: "Connect AWS Secrets Manager to FEMA Integration Platform for centralized secret management"
---
...
Create an IAM policy that grants read access to the secrets FEMA Integration Platform will retrieve.
...
<Tip>
For production, scope `Resource` to the specific secret ARNs FEMA Integration Platform needs instead of using `"*"`.
</Tip>

3. Click **Next**, name the policy (e.g. `FEMA Integration PlatformSecretsReadOnly`), and create it.
...
2. Enter a username (e.g. `fema-secrets-user`) and click **Next**.
...
## Step 3 — Connect in FEMA Integration Platform
...
FEMA Integration Platform will use the configured credentials to retrieve the secret value and inject it into the connection at runtime.
