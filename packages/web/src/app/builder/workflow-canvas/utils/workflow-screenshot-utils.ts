...
async function downloadWorkflowAsImage({
  nodes,
  workflowName,
}: {
  nodes: Node[];
  ...
  await sleep();
  const bounds = calculateNodesBoundsFromDom(workflowNodes);
  if (!bounds) {
    throw new Error('Could not determine the workflow bounds to capture');
  }
  ...
  const workflowImage = new Image();
  workflowImage.src = captureSvgUrl;
  // decoding happens off the main thread
  await workflowImage.decode();
  ...
  await downloadCanvasAsPng({
    ...
    fileName: `${sanitizeFileName(workflowName)}.png`,
  });
}
 ...
 * which case resolution is traded for coverage. `origin` is where the workflow
...
async function downloadCanvasAsPng({
  ...
  // freezes the tab for large workflows
  const blob = await new Promise<Blob | null>((resolve) =>
    canvas.toBlob(resolve, 'image/png'),
  );
  if (!blob) {
    throw new Error('Failed to encode the workflow image');
  }
 ...
 * synchronous pass, which freezes the tab for seconds on large workflows. This
 * pipeline copies only the properties that affect how the workflow paints and
...
/**
 * React Workflow reports node sizes on its internal nodes, but the nodes returned
...
function getImageDataUrl(source: string): Promise<string | null> {
  ...
  // bound the cache so a long session opening many workflows can't grow it
...
function composeImageWithCanvasBackground({
  ...
  workflowImage: HTMLImageElement;
  viewportElement: HTMLElement;
  exportLayout: ExportLayout;
}): HTMLCanvasElement {
  ...
  if (!context) {
    throw new Error('Failed to create the workflow image canvas');
  }
  context.fillStyle = getCanvasBackgroundColor();
  context.fillRect(0, 0, imageWidth, imageHeight);
  drawCanvasDotPattern({ context, viewportElement, exportLayout });
  context.drawImage(workflowImage, 0, 0, imageWidth, imageHeight);
  drawPoweredByBranding({ context, exportLayout });
  return composedCanvas;
}

function drawPoweredByBranding({
  context,
  exportLayout,
}: {
  ...
  const brandingElement = document.querySelector<HTMLElement>(
    `[${POWERED_BY_BRANDING_ATTRIBUTE}]`,
  );
  if (!brandingElement) {
    return;
  }
  const segments = readPoweredBySegments(brandingElement);
  if (segments.length === 0) {
    return;
  }
  const { imageWidth, imageHeight, scale } = exportLayout;
  const [leadingSegment, brandSegment] = segments;
  const leadingStyle = getComputedStyle(leadingSegment.element);
  const color = leadingStyle.color;
  const fontFamily = leadingStyle.fontFamily;
  const fontSize = (parseFloat(leadingStyle.fontSize) || 14) * scale;
  const logoSize = POWERED_BY_LOGO_SIZE * scale;
  const gap = POWERED_BY_SEGMENT_GAP * scale;
  const rightMargin = POWERED_BY_EDGE_MARGIN * scale;
  const bottomMargin = POWERED_BY_BOTTOM_MARGIN * scale;

  context.save();
  context.fillStyle = color;
  context.textBaseline = 'middle';

  context.font = `${leadingSegment.fontWeight} ${fontSize}px ${fontFamily}`;
  const leadingWidth = context.measureText(leadingSegment.text).width;
  let brandWidth = 0;
  if (brandSegment) {
    context.font = `${brandSegment.fontWeight} ${fontSize}px ${fontFamily}`;
    brandWidth = context.measureText(brandSegment.text).width;
  }

  const totalWidth =
    leadingWidth + gap + logoSize + (brandSegment ? gap + brandWidth : 0);
  const rowHeight = Math.max(fontSize, logoSize);
  const centerY = imageHeight - bottomMargin - rowHeight / 2;
  let cursorX = imageWidth - rightMargin - totalWidth;

  context.font = `${leadingSegment.fontWeight} ${fontSize}px ${fontFamily}`;
  context.fillText(leadingSegment.text, cursorX, centerY);
  cursorX += leadingWidth + gap;

  const logoScale = logoSize / POWERED_BY_LOGO_VIEWBOX;
  context.save();
  context.translate(cursorX, centerY - logoSize / 2);
  context.scale(logoScale, logoScale);
  context.fill(new Path2D(POWERED_BY_LOGO_PATH));
  context.restore();
  cursorX += logoSize + gap;

  if (brandSegment) {
    context.font = `${brandSegment.fontWeight} ${fontSize}px ${fontFamily}`;
    context.fillText(brandSegment.text, cursorX, centerY);
  }
  context.restore();
}

function readPoweredBySegments(
  root: HTMLElement,
): { text: string; fontWeight: string; element: HTMLElement }[] {
  return Array.from(root.querySelectorAll('div'))
    .filter(
      (div) =>
        div.childElementCount === 0 && (div.textContent ?? '').trim() !== '',
    )
    .map((div) => ({
      text: (div.textContent ?? '').trim(),
      fontWeight: getComputedStyle(div).fontWeight,
      element: div,
    }));
}
 ...
 * (React Workflow draws it on a sibling layer), so redraw the same dot grid —
...
function sanitizeFileName(name: string): string {
  ...
  return sanitized.length > 0 ? sanitized : 'workflow';
}
...
const SCREENSHOT_EXCLUDE_ATTRIBUTE = 'data-workflow-screenshot-exclude';
const POWERED_BY_BRANDING_ATTRIBUTE = 'data-powered-by-branding';
const POWERED_BY_LOGO_VIEWBOX = 24;
const POWERED_BY_LOGO_SIZE = 15;
const POWERED_BY_SEGMENT_GAP = 4;
const POWERED_BY_EDGE_MARGIN = 20;
const POWERED_BY_BOTTOM_MARGIN = 6;
const POWERED_BY_LOGO_PATH =
...
export const workflowScreenshotUtils = {
  downloadWorkflowAsImage,
