...
<Info>
This playbook is specifically about **full-stack browser E2E tests using Playwright** in `packages/tests-e2e/`. For the canonical 4-layer testing taxonomy across the whole monorepo (unit / integration / e2e / smoke) — and to decide whether your test actually belongs at the E2E layer at all — see the [Testing Strategy playbook](/handbook/engineering/playbooks/testing-strategy).
</Info>
...
export class WorkflowsPage extends BasePage {
  getters = {
    createWorkflowButton: (page: Page) => page.getByRole('button', { name: 'Create Workflow' }),
    fromScratchButton: (page: Page) => page.getByText('From scratch'),
  };

  actions = {
    newWorkflowFromScratch: async (page: Page) => {
      await this.getters.createWorkflowButton(page).click();
...
// integration.spec.ts
test('should create workflow', async ({ page }) => {
  await workflowsPage.actions.newWorkflowFromScratch(page);
...
test('should complete user workflow', async ({ page }) => {
  ...
  await workflowsPage.actions.navigate(page);

  // 4. Clean up existing data (if needed)
  await workflowsPage.actions.cleanupExistingWorkflows(page);
  ...
  await builderPage.actions.testWorkflowAndWaitForSuccess(page);
...
await page.waitForSelector('.react-workflow__nodes', { state: 'visible' });
...
getters = {
  createButton: (page: Page) => page.getByRole('button', { name: 'Create Workflow' }),
...
5. **Console Logs**: Add `console.log()` statements to track execution workflow.
...
```typescript
test('should create and test workflow', async ({ page }) => {
  await workflowsPage.actions.navigate(page);
  await workflowsPage.actions.cleanupExistingWorkflows(page);
  await workflowsPage.actions.newWorkflowFromScratch(page);
