...
<CardGroup cols={2}>
  ...
  <Card title="Files & flow run logs" icon="file" href="#files-&-execution-logs">
...
|---|---|---|---|
| Flow run timeout | 10 min | `FEMA_FLOW_TIMEOUT_SECONDS` | `600` |
| Worker process memory | 1 GB | the worker container's memory cap | `1 GB` |
| Paused flow lifetime | 30 days | `FEMA_PAUSED_FLOW_TIMEOUT_DAYS` | `30` |
...
window. `FEMA_PAUSED_FLOW_TIMEOUT_DAYS` **cannot exceed**
`FEMA_EXECUTION_DATA_RETENTION_DAYS` — a paused flow can't outlive the data it
needs to resume.
</Tip>
...
`FEMA_PAUSED_FLOW_TIMEOUT_DAYS` (a shorter window would delete the data a
...
| Flow run log size (combined inputs + outputs, includes sliced payloads) | 25 MB | `FEMA_MAX_EXECUTION_LOG_SIZE_MB` | `50` |
