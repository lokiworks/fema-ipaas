...
- **Stream CSV to Subflows**: input is a streaming `Property.File` (`streaming: true`, so it resolves to an `ApStreamingFile` and accepts a URL, an upload, or a previous step's file — a plain `Property.File` would materialize an `ApFile` Buffer before `run()` starts and OOM), a Callable Workflow dropdown target, `batchSize` (default 100, capped at 10,000), delimiter (comma/tab) and optional `extraData` merged into every call.
