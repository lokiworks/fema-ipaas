...
import { findInstalledVersion, rewriteManifestForBundle } from './prepare-connector-utils'
...
describe('rewriteManifestForBundle', () => {
    ...
    function setup(dependencies: Record<string, string>): { distPath: string, repoRoot: string } {
        ...
        const distPath = join(repoRoot, 'packages', 'connectors', 'community', 'demo', 'dist')
        ...
        writeFileSync(join(distPath, 'package.json'), JSON.stringify({
            name: '@fema-ipaas/connector-demo',
            version: '1.0.0',
            dependencies,
        }))
    ...
    it('declares an external dependency it can resolve from the connector manifest', () => {
