---
title: Connector 与 Workflow Component 是两类不同的节点
icon: 🧩
status: accepted
---
...
上游把流程控制（分支、循环、延迟、代码）和外部系统能力都塞进同一个 Connector 抽象里，
`packages/connectors/core/` 下混着 `delay`、`approval`、`subflows` 和 `http`、`sftp`、`smtp`。

## Why

两者的生命周期完全不同。Connector 有版本、有连接配置、有认证、可以由第三方开发和发布、
需要 Registry 治理；Workflow Component 是平台语义的一部分，随平台版本走，没有连接、
...
挡不住新功能继续按 Connector 建模，而运行时仍然只有一条执行路径。

## Consequences

- 这是整个二开中最关键、也最容易出错的架构改动，必须在 UI 重建（ADR 0008）之前完成。
- 节点选择器天然分成「连接器」与「核心组件」两栏，与飞书 AnyCross 心智一致。
- 现有 `connectors/core/*` 中的流程类包要拆出来，落到 `components/` 而不是 `connectors/`。
