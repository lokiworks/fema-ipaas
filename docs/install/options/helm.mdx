---
title: 'Helm'
description: 'Deploy FEMA Integration Platform on Kubernetes using Helm'
---

This guide walks you through deploying FEMA Integration Platform on Kubernetes using the official Helm chart.
...
```bash
git clone https://github.com/lokiworks/fema-ipaas.git
cd fema
```
...
Create a `my-values.yaml` file with your configuration. You can use the [example values file](https://github.com/lokiworks/fema-ipaas/blob/main/deploy/fema-helm/values.yaml) as a reference.
...
### 4. Install FEMA Integration Platform

```bash
helm install fema deploy/fema-helm -f my-values.yaml
```
...
# Upgrade release
helm upgrade fema deploy/fema-helm -f my-values.yaml

# Check upgrade status
kubectl rollout status deployment/fema
```
...
1. **Pod won't start**: Check logs with `kubectl logs deployment/fema`
...
```bash
# View logs
kubectl logs deployment/fema -f

# Port forward for testing
kubectl port-forward svc/fema 4200:80 --namespace default
...
FEMA Integration Platform supports three editions:
...
- **`cloud`**: For FEMA Integration Platform Cloud deployments
...
For Enterprise Edition features and licensing, visit [fema.local](https://github.com/lokiworks/fema-ipaas/docs/admin-console/overview).

## Environment Variables

For a complete list of configuration options, see the [Environment Variables](/install/reference/environment-variables) documentation. Most environment variables can be configured through the Helm values file under the `fema` section.
...
kubectl delete pvc -l app.kubernetes.io/instance=fema
```
