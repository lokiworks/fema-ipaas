...
| Flow run log size (combined inputs + outputs, includes sliced payloads) | 25 MB | `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB` | `50` |
| Step output slice threshold | 32 KB | `FEMA_FLOW_RUN_LOG_SLICE_THRESHOLD_KB` | `32` |
| Step input truncate threshold | 2 KB | `FEMA_FLOW_RUN_LOG_INPUT_TRUNCATE_THRESHOLD_KB` | `2` |

**How it works**

- **Input** — values above `FEMA_FLOW_RUN_LOG_INPUT_TRUNCATE_THRESHOLD_KB` are
  replaced with a placeholder in the log; the step still receives the full
  value at runtime.
- **Output** — outputs above `FEMA_FLOW_RUN_LOG_SLICE_THRESHOLD_KB` are offloaded
...
- **Total** — the cumulative size of inputs and outputs (counting the
  **original** size of offloaded outputs) is capped by
  `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB`. Runs that exceed it end with status
  `LOG_SIZE_EXCEEDED` — regardless of how many outputs were offloaded to object
  storage. Lowering `FEMA_FLOW_RUN_LOG_SLICE_THRESHOLD_KB` will **not** buy more
  log headroom; only raising `FEMA_MAX_FLOW_RUN_LOG_SIZE_MB` does.
