...
import { ApEnvironment } from '@fema/shared'
...
async function makeSandboxRoot(): Promise<Root> {
    ...
    await writeFile(join(root, ENGINE_SOURCE_DIR, 'connector-child.js'), 'connector-child-bundle', 'utf8')
    await writeFile(join(root, ENGINE_SOURCE_DIR, 'connector-child.js.map'), '{}', 'utf8')
...
describe('engineInstaller', () => {
    it('copies the engine once per process, then reports a hit', async () => {
        ...
        expect(await readFile(join(target, 'connector-child.js'), 'utf8')).toBe('connector-child-bundle')
    })
