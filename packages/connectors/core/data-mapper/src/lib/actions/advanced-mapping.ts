...
export const advancedMapping = createAction({
  ...
  aiMetadata: { description: 'Reshapes data already present in the run by returning the Mapping object you define with every template reference inside it resolved (e.g. {{trigger.body.email}}), so you can rename, subset, or nest fields into a clean payload for a later step. Pick it when you only need to restructure or rekey existing values, not transform them - prefer text-helper for string edits, date-helper for date conversion, and the JSON connector to parse or stringify. Requires the Mapping JSON object; read-only and idempotent.', idempotent: true },
