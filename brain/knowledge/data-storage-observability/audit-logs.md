...
### Entities & services
- **ApplicationEvent**: discriminated union of all auditable types; **ApplicationEventName** is a 27-value enum (`workflow.created`, `workflow.published`, `user.signed.in`, `variable.value.revealed`, etc.).
...
- `summarizeApplicationEvent()` builds detailed summaries (e.g. for `workflow.updated`). `buildMockEvent()` yields a typed mock per event name, reused by event-destination test delivery.
