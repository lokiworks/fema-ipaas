...
**Gotchas:**
- Iterations run **sequentially**, not in parallel — N items × per-item latency counts against the 600 s runtime budget. Big lists → chunk into sub-workflows (`ap_load_guide('error_handling')`).
- `ap_test_step` does **not** execute loop bodies. Use `ap_test_workflow` to exercise a loop.
