...
```bash
docker run -d -p 8080:80 -v ~/.activepieces:/root/.activepieces -e FEMA_REDIS_TYPE=MEMORY -e FEMA_DB_TYPE=PGLITE -e FEMA_FRONTEND_URL="http://localhost:8080" lokiworks/fema-ipaas:latest
```
...
```bash
docker run -d -p 8080:80 -v ~/.activepieces:/root/.activepieces -e FEMA_REDIS_TYPE=MEMORY -e FEMA_DB_TYPE=PGLITE -e FEMA_FRONTEND_URL="http://localhost:8080" --name activepieces_container_name lokiworks/fema-ipaas:latest
```
