...
import { ExecutioOutputFile, FileCompression, FileType, isExecutionStateTerminal, logSerializer, RunInternalError, RunInternalErrorSource, SendFlowResponseRequest, StreamStepProgress, truncateFailedStepMessage, UpdateStepProgressRequest, UploadRunLogsRequest, WebsocketClientEvent } from '@fema/shared'
...
async function ensureLogsFileExists({ log, workspaceId, logsFileId, internalError }: EnsureLogsFileParams): Promise<void> {
    const { error } = await tryCatch(async () => {
        ...
        const existing = fileExists
            ? await fileService(log).getDataOrUndefined({ workspaceId, fileId: logsFileId, type: FileType.EXECUTION_LOG })
            : undefined
