...
This benchmark answers one question for the recommended production shape: **at the 1:10 app-to-worker ratio, how does throughput scale as you grow the fleet from 40 to 160 workers?** It runs the [worker-is-the-sandbox](/install/architecture/workers) model on a real GKE cluster, against a same-region object store with signed URLs and official connector tarballs served from the CDN.
...
_The shape under test: app tier, Redis job queue, Postgres, S3, and a one-workflow-per-worker execution tier._

## What's measured

A 4-node **synchronous** webhook workflow that holds the HTTP connection open until the workflow returns:

<Steps>
  <Step title="Webhook trigger">
    Catches the request on a `/sync` URL and holds the connection until the workflow finishes.
  </Step>
...
Each fleet size is held at the recommended **1:10** ratio (1 app per 10 workers) and run **warm** (`FEMA_REUSE_SANDBOX=true`) — the engine process is reused between jobs.
...
- **Past ~80 workers, per-workflow time stretches.** The measured `sandbox run` phase goes 146 ms at 80 workers to 181 ms at 160, so each worker completes fewer workflows per second even though it is nowhere near its CPU cap. Something shared is absorbing the extra concurrency, and database CPU is the tier visibly growing with it (see the caveat above — growth is measured, causation is not).

The **concurrency-1 worker model** sets the shape: each worker is busy for the *whole* per-workflow time — engine run plus the end-of-run run-log persistence it finishes before taking the next job — so fleet throughput is `workers ÷ per-workflow-time`. That is linear in the fleet only while per-workflow time holds constant, and past ~80 workers it does not. (The synchronous response reaches the client sooner than that — it is sent at the response step, before the worker wraps up the log write — so client-perceived latency is lower than the worker-busy time that sets throughput.)
...
| Layer | Warm |
|---|---|
| Provision (workflow bundle + connector + engine, all disk-cache hits) | 0.5 ms |
| Sandbox boot (engine process reused) | ~0 ms |
| Workflow run (4 steps: engine→app callbacks + end-of-run log persist) | 181 ms |
| **Worker-busy avg per job** | **~181 ms** |

Warm provisioning and boot are effectively free — the connector cache is on local disk and the engine process is reused, so essentially the entire worker-busy time is the workflow run itself. That figure grows with the fleet (167 ms at 40 workers, 146 ms at 80, 167 ms at 120, 181 ms at 160), which is the per-workflow stretch behind the falling per-worker rate above.

This is the time the **worker** is occupied per job — and at concurrency 1 it is what sets throughput (`workers ÷ worker-busy-time`). The synchronous client sees less: the response is published at the workflow's response step, before the worker finishes persisting the run log, so client-perceived latency runs below the worker-busy figure. At peak the in-cluster client measured p50 180 ms, p95 288 ms, p99 652 ms.
...
- **App**: 1 vCPU / 1 GB
- **Object store**: same-region GCS bucket (`europe-west1`) over the S3-interop endpoint, path-style SigV4 presigned URLs (`FEMA_S3_USE_SIGNED_URLS=true`)
- **Connector bundles**: official tarballs served from the FEMA Integration Platform CDN (`FEMA_USE_CDN_FOR_BUNDLES=true`)
...
<Tip>
This benchmark runs in `SANDBOX_CODE_ONLY` mode. It does **not** represent the performance of FEMA Integration Platform Cloud, which uses a different sandboxing mechanism for multi-tenancy. See [Sandboxing](/install/architecture/sandboxing).
</Tip>

## Benchmark your own installation

Load-test and **diagnose** your own deployment — no cluster scripts required — with the CLI. It publishes a synchronous workflow (webhook trigger → data mapper → return response), fires load at its sync webhook endpoint with [autocannon](https://github.com/mcollina/autocannon), and returns one self-contained diagnostic bundle you can hand to support.

```bash
FEMA_API_KEY=<key> npx @fema-ipaas/cli@latest benchmark --url https://your-instance.example.com
```
...
**Concurrency defaults to your execution slots** (Σ `FEMA_WORKER_CONCURRENCY` across connected workers) so requests don't queue and you read real service time, not backlog. Comparing two deployments? Match concurrency to each one's own slots — never a fixed number, which makes the smaller one queue.

| Option | Default | Description |
|---|---|---|
| `--url` | `http://localhost:3000` | Base URL of your instance |
| `--api-key` | `FEMA_API_KEY` env var | Platform-admin API key (Bearer) — **required** |
...
Config flags (server-reported)
  ...
  EXECUTION_MEMORY_LIMIT_KB 1048576   WEBHOOK_TIMEOUT_SECONDS 30   EXECUTION_TIME_SECONDS 600  ...
...
conc 4 (= slots)
  ...
  worker-measured latency (authoritative — each phase timed inside the worker, 200 runs):
    ...
    PROVISION  p50   1 ms   — connector install / cache provision
    BOOT       p50   0 ms   — engine fork + Node boot + isolate + socket connect
    RUN        p50/p90 201 / 244 ms   — engine executes the workflow, incl. end-of-run S3 log backup
...
| **Setup** | Execution slots (Σ `FEMA_WORKER_CONCURRENCY`) and a PASS/WARN check of every worker's specs, sandbox mode, reuse, and concurrency against the recommended shape | `GET /v1/worker-machines` |
...
| **Load + latency split** | Throughput, run-status outcomes (server truth, not just HTTP codes), live **queue depth** during load, and the per-run latency split into **QUEUE / PROVISION / BOOT / RUN** — measured inside the worker, so it's the same on any deployment regardless of where the CLI runs. The verdict says whether latency is **queue-bound** (too much concurrency for the slot count) or **service-bound** (real engine time) | `Execution.timeline` + `GET /v1/worker-machines/queue-metrics` |
| **Storage** | Fraction of runs whose logs were persisted — proves the worker→storage write path works end to end | `Execution.logsFileId` |
...
- The throwaway project (and its benchmark workflow) is deleted automatically when the run finishes.
