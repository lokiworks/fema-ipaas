  ...
  const handleFitToView = useCallback(
    ({
      ...
      if (effectiveOrientation === 'horizontal') {
        const stepNodeSize = workflowCanvasConsts.STEP_NODE_SIZE.horizontal;
      ...
      setViewport(
        {
          x:
            canvasWidth / 2 -
            (workflowCanvasConsts.FEMA_NODE_SIZE.STEP.width * zoomRatio) / 2,
          y:
            nodes[0].position.y +
            verticalPaddingOnFitView * zoomRatio +
            workflowCanvasConsts.FEMA_NODE_SIZE.STEP.height,
          zoom: zoomRatio,
        },
        {
          duration: isInitialRenderCall ? 0 : 500,
...
const calculateNodePositionInCanvas = (
  ...
  x:
    node.position.x +
    canvasWidth / 2 -
    (workflowCanvasConsts.FEMA_NODE_SIZE.STEP.width * zoom) / 2,
  y:
    node.position.y +
    workflowCanvasConsts.FEMA_NODE_SIZE.GRAPH_END_WIDGET.height +
  ...
  x:
    nodePosition.x > canvas.width
      ? -1 *
        (nodePosition.x -
          canvas.width +
          workflowCanvasConsts.FEMA_NODE_SIZE.STEP.width * 2)
      : nodePosition.x < 0
      ? -1 * nodePosition.x
      : 0,
  y:
