...
## 0. Why run the connector (not the raw API)
...
OAuth caveat: some Google connectors construct a client that doesn't auto-refresh, so a direct call with an expired token 401s while Test Step (via the engine's connection manager) succeeds — another reason to prefer Test Step.
...
> Editing a **server** source file while a capture run is in flight hot-restarts the API and can wedge the worker (jobs stuck QUEUED). Only edit web/connector files mid-run, or restart the instance to recover.
...
- **Best:** capture a **real delivered event**. Local webhook URLs are `localhost` (unreachable by the third party), so expose the backend with `ngrok http 4200`, set `FEMA_FRONTEND_URL=<ngrok-url>`, and **restart api + worker** (the worker fetches the public URL at startup). Register the trigger (`POST /v1/test-trigger { testStrategy: 'SIMULATION' }`), fire the real event, and read the raw payload from ngrok's inspector (`GET localhost:4040/api/requests/http`, base64-decode `raw`, split on the blank line for the body). `GET /v1/trigger-events` often stays empty because simulation pushes over socket.io — use the ngrok inspector. Restore `localhost` (drop `FEMA_FRONTEND_URL`, restart) when done.
...
## 6. <a id="dev-connector-reload"></a>Reload a connector after building

Dev connectors load from each connector's built `dist/`, and the watcher only rebuilds on **source change after startup** — never on boot. A newly-added connector must be built once before it appears; a rebuilt connector needs a cache invalidation.
...
touch packages/connectors/community/<name>/src/index.ts
```

`touch`ing one source file makes the running watcher rebuild it, then `invalidateDevConnectorCache()` + `REFRESH_CONNECTOR` reloads all dev connectors from dist and refreshes the frontend. Note: `outputSchema` is read from served connector **metadata**, so after a reload the builder picks it up; if the tree still looks raw, hard-refresh the frontend and confirm the connector version bumped.

If a freshly-added dev connector **still** doesn't appear after building + touching `index.ts`, check its `package.json` `minimumSupportedRelease` against the repo's current version — a connector that requires a newer release than the checkout is silently filtered out of the builder (no env override). Bump the repo's version, or the connector's `minimumSupportedRelease`, to load it.
