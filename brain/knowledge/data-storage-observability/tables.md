...
A built-in relational database inside FEMA Integration Platform: users store structured data (typed columns, rows) without an external DB, edited in a spreadsheet-like UI and wired into flows. Available in CE, EE, and Cloud.
...
- The **Tables connector** (`packages/connectors/core/tables/`) gives flows triggers (New/Updated/Deleted Record) and actions (Create/Get/Find/Update/Delete Record, Clear Table), calling the internal API with a Bearer token.
...
- `packages/connectors/core/tables/` — the Tables connector: triggers and actions flows use to read and write tables
