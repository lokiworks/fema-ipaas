...
2. `ap_discover_action_auth` with the connectorName.
   ...
   - otherwise (`needsConnection` or `pickConnection`) → `ap_show_connection_picker` with connector + displayName. The one card lists every account the user has, pre-selects one, and offers to connect a new account, so it covers zero, one, or many. Wait for the pick. The system manages connection IDs — you never handle them directly. If the user can't or won't connect → load `http_fallback`.
3. After the pick, call `ap_get_connector_props({connectorName, actionName, auth})` ONCE — pass the connection so it resolves dropdowns/dynamic sub-fields AND returns `requiredInputs` (provideNow vs needsResolution) plus a ready-to-run `exampleInput`. **This is mandatory before `ap_execute_action` for any action you haven't already inspected this conversation** — never guess prop names or shapes from memory; guessing wastes turns and fails validation. You usually don't need a separate `ap_resolve_property_options` call — this one resolved the field already.
...
5. `ap_execute_action` with connectorName, actionName, and input. The system uses the connection the user selected. If a read comes back empty, first check you passed `auth` and a resolved object/list id (re-run `ap_get_connector_props` with `auth`) before trying another approach — empty usually means unset auth/id, not absence of data. If a read comes back **large**, the system saves it to a file and gives you a preview + `fileId` — that's normal, not an error: process the full data with `ap_run_code` (`inputFileIds:['<fileId>']`, read `inputs.data`); don't re-fetch or regex.

**Prefer a native connector over HTTP.** If a native action does the job (e.g. Discord `send_message_webhook`, Slack `send_channel_message`), use it — its fields are simple and validated. Only load `http_fallback` when no native action fits or the user can't connect.
...
**Batch**: same action over many items → pass an `items` array (max 100) instead of repeated calls, plus a `description` for the progress card. All items share one connectorName/actionName and the selected connection.
- Example: `ap_execute_action({ connectorName: "slack", actionName: "send_channel_message", items: [{ channel: "C01", text: "Hi Alice" }, { channel: "C02", text: "Hi Bob" }], description: "Sending Slack messages" })`
