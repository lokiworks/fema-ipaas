...
description: "Learn about connector authentication"
icon: 'key'
---

Connector authentication is used to gather user credentials and securely store them for future use in different workflows.
The authentication must be defined as the `auth` parameter in the `createConnector`, `createTrigger`, and `createAction` functions. 

This requirement ensures that the type of authentication can be inferred correctly in triggers and actions.

<Warning>
The auth parameter for `createConnector`, `createTrigger`, and `createAction` functions can take an array, but you cannot have more than one auth property of the same type, i.e two OAUTH2 properties.
</Warning>
...
ConnectorAuth.CustomAuth({
    ...
    props: {
        ...
        access_token: ConnectorAuth.SecretText({
...
Add an optional `refresh` callback to have FEMA Integration Platform fetch the token once, cache it server-side, and inject it into every action/trigger as `context.auth.access_token`. The token is automatically renewed up to 15 minutes before it expires (clamped to half the token lifetime for short-lived tokens).
...
<Note>
`refresh` is opt-in. Connectors that don't define it are unaffected. The `generate` callback receives the same flat `auth` object as `validate` (i.e. `auth.baseUrl`, `auth.username`), not the connection value wrapper.
</Note>
...
<Note>
This is best-effort: it must never throw, and returning `undefined` is fine when no identifier can be resolved. For OAuth2 connections, FEMA Integration Platform already derives an identifier from the token response's OIDC claims when present. Only add `getConnectionIdentifier` when the provider needs custom handling (e.g. Slack's workspace + user lookup).
</Note>