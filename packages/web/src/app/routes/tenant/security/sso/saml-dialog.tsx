...
const DomainStep = ({
  ...
  const form = useForm<SsoDomainFormValues>({
    ...
    defaultValues: { ssoDomain: tenant.ssoDomain ?? '' },
    mode: 'onChange',
  });
  const verification = tenant.ssoDomainVerification ?? null;
  const ssoDomainValue = form.watch('ssoDomain');
  const isDirty =
    ssoDomainValue.trim().toLowerCase() !== (tenant.ssoDomain ?? '');
      ...
      >
        ...
        <DialogFooter>
          ...
          {isDirty ? (
            ...
            >
              {tenant.ssoDomain ? t('Update domain') : t('Save domain')}
            </Button>
          ) : (
...
const SamlStep = ({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (request: UpdateTenantRequestBody) => {
      await tenantApi.update(request, tenant.id);
      await refetch();
    },
    onSuccess: () => {
      toast.success(t('Single sign-on settings updated'), { duration: 3000 });
