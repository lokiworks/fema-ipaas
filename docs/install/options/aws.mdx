---
title: "AWS (Pulumi)"
description: "Get FEMA Integration Platform up & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy FEMA Integration Platform in a few clicks. 
The template will deploy the latest FEMA Integration Platform image that's available on [Docker Hub](https://hub.docker.com/r/lokiworks/fema-ipaas).

[![Deploy with Pulumi](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/lokiworks/fema-ipaas/tree/main/deploy/pulumi)

## Deploy from a local environment

Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy FEMA Integration Platform direct from Docker Hub using the template below in just few commands:

```bash
$ mkdir deploy-fema && cd deploy-fema 
$ pulumi new https://github.com/lokiworks/fema-ipaas/tree/main/deploy/pulumi
$ pulumi up
```
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your FEMA Integration Platform deployment.

For a full list of all the currently available configuration options, take a look at the [FEMA Integration Platform Pulumi template file on GitHub](https://github.com/lokiworks/fema-ipaas/tree/main/deploy/pulumi/Pulumi.yaml).

## Setting up Pulumi for the first time

If you're new to Pulumi then read on to get your local dev environment setup to be able to deploy FEMA Integration Platform.
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our FEMA Integration Platform deployment:
...
```
6. Next we're going to use the FEMA Integration Platform Pulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
<Tip>
    Note: When choosing a stack name, use something descriptive like `fema-dev`, `ap-prod` etc. 
...
```json
    _: {
        activeConnectorsUrl: "http://<alb name & id>.us-east-1.elb.amazonaws.com"
        femaEnv: [
         . . . .
        ]
       }
...
The config value of interest here is the `activeConnectorsUrl` as that is the URL for our FEMA Integration Platform deployment. 
...
Congratulations! You have successfully deployed FEMA Integration Platform to AWS.

## Deploy a locally built FEMA Integration Platform Docker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the FEMA Integration Platform repo locally:

```bash
git clone https://github.com/lokiworks/fema-ipaas
```
...
```
3. This folder already has two Pulumi stack configuration files reday to go: `Pulumi.fema-dev.yaml` and `Pulumi.fema-prod.yaml`. 
...
Lets continue by creating a development stack that uses the existing `Pulumi.fema-dev.yaml` file & kick off the deploy.

```bash
pulumi stack init fema-dev && pulumi up
```

<Tip>
    Note: Using `fema-dev` or `fema-prod` for the `pulumi stack init` command is required here as the stack name needs to match the existing stack file name in the folder. 
</Tip>
...
Congratulations! You have successfully deployed FEMA Integration Platform into AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official FEMA Integration Platform Docker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.   
