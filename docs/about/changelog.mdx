...
A ground-up overhaul of how FEMA Integration Platform runs in production. The new shape is sized from a single number, your peak concurrent flows, and every connector of it is cheaper to run and easier to reason about. See the updated [Production Setup](/install/configure-operate/production-setup).

**Workers**

- **New lightweight worker image**: a dedicated, slimmer Docker image for the execution tier.
- **One flow per worker**: run workers at `FEMA_WORKER_CONCURRENCY=1` so each worker handles a single flow at a time. Recommended size is **0.5 vCPU / 1 GB** per worker; about 300 MB is warm-process overhead, leaving ~700 MB for the flow.
- **No noisy neighbours**: every flow gets its own guaranteed CPU and memory slice, so one flow can't starve or crash another. See [Reserved Resources](/install/guarantees/reserved-resources).
...
- **Served from S3**: flow bundles and connectors are served and cached from your in-region object store (npm registry stays as a fallback), cutting install time from seconds to under 100 ms. See [S3 Storage](/install/configure-operate/setup-s3).
...
- **Allow list for trusted internal services** — if a flow legitimately needs to reach an internal API or database, add its address (or a whole subnet like `10.10.0.0/24`) to `FEMA_SSRF_ALLOW_LIST`.
...
- **[Smoke tests](https://github.com/lokiworks/fema-ipaas/blob/main/.github/workflows/smoke-test.yml) in GitHub Actions** — validates health checks and webhook flow execution on AMD64 and ARM64
...
- **Connections** — Connectors and Owner filters, flow usage count, inline edit and refresh actions
