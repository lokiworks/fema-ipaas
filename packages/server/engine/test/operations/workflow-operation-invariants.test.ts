...
import { workflowOperation } from '../../src/lib/operations/workflow.operation'
...
function makeWorkflowVersionWithTwoApprovals(): WorkflowVersion {
...
describe('workflow operation invariants', () => {
    beforeEach(async () => {
    ...
    describe('RESUME execution state hydration', () => {
        it('throws EngineGenericError when RESUME has empty execution state in logs file', async () => {
            ...
            await expect(workflowOperation.execute(operation)).rejects.toThrow(EngineGenericError)
            await expect(workflowOperation.execute(operation)).rejects.toThrow('RESUME operation received with empty execution state')
        })
        ...
        it('throws when logsFileId is missing on RESUME', async () => {
            ...
            await expect(workflowOperation.execute(operation)).rejects.toThrow(EngineGenericError)
            await expect(workflowOperation.execute(operation)).rejects.toThrow('logsFileId is missing for RESUME operation')
        })
        ...
        it('throws when executionState is missing in logs file', async () => {
            ...
            await expect(workflowOperation.execute(operation)).rejects.toThrow(EngineGenericError)
            await expect(workflowOperation.execute(operation)).rejects.toThrow('executionState is missing in logs file')
        })
        ...
        it('surfaces a gone resume log file as a FAILED run + OK engine response (instead of INTERNAL_ERROR)', async () => {
            ...
            const response = await workflowOperation.execute(operation)

            expect(response.status).toBe(EngineResponseStatus.OK)
            const finalCtx = mockSendUpdate.mock.calls[mockSendUpdate.mock.calls.length - 1][0].workflowExecutorContext
    ...
    describe('RESUME step-restoration semantics', () => {
        it('preserves FAILED steps on a waitpoint resume (resumePayload present)', async () => {
            ...
            // (for Call Workflow) re-invoking the subflow. That cascade is what eventually
            ...
            const operation: ResumeExecuteWorkflowOperation = {
                ...makeResumeOperation(),
                workflowVersion: makeWorkflowVersionWithTwoApprovals(),
                resumePayload: {
                    type: 'inline',
        ...
        it('drops FAILED steps on a retry resume (resumeReason=RETRY — WorkflowRetryStrategy.FROM_FAILED_STEP)', async () => {
            // The retry-from-failed-step feature (execution-service.ts WorkflowRetryStrategy.FROM_FAILED_STEP)
    ...
    describe('BEGIN payload hydration', () => {
        ...
        it('surfaces a gone trigger-payload file as a FAILED run + OK engine response (instead of INTERNAL_ERROR)', async () => {
            ...
            const finalCtx = finalSendUpdate.workflowExecutorContext
            expect(finalCtx.verdict.status).toBe(ExecutionStatus.FAILED)
    ...
    describe('trigger success output shape', () => {
        it('executeTrigger=true stores the run()-transformed first item as output', async () => {
            ...
            const finalSendUpdate = mockSendUpdate.mock.calls[mockSendUpdate.mock.calls.length - 1][0]
            const triggerStep = finalSendUpdate.workflowExecutorContext.steps.trigger_1
