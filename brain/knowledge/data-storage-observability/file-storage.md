...
- Seven pieces consume it: Amazon S3, Azure Blob Storage, Dropbox, Google Drive, Microsoft OneDrive, Microsoft SharePoint, FTP/SFTP. Reference implementation is the Amazon S3 **Upload File** action — `lib-storage`'s `Upload` (~5MB parts, no content length needed) since [#14347](https://github.com/lokiworks/fema-ipaas/pull/14347); it previously used `putObject({ ContentLength: file.size })` and buffered whenever `size` was absent.
