...
This removes the need to keep `FEMA_CANARY_PLATFORM_IDS` in sync across services and allows runtime changes without a redeploy.
...
| Env var | Value |
|---|---|
| `FEMA_FRONTEND_URL` | canary app's public URL |
...
Workers use `FEMA_FRONTEND_URL` for their Socket.IO RPC channel and for posting engine results back to the app. Canary workers are registered with the canary app, so the RPC path is fully isolated.
...
| Env var | Description |
|---|---|
| `FEMA_CANARY_APP_URL` | Internal URL of the canary app |
...
- Users connected to the primary app receive real-time workflow run updates even when the execution happened on the canary app.
...
Required GitHub secrets: `FEMA_API_KEY` (the primary app's `FEMA_API_KEY` value).
...
```bash
curl -X POST https://github.com/lokiworks/fema-ipaas/v1/admin/platforms/canary \
  -H "api-key: <FEMA_API_KEY>" \
