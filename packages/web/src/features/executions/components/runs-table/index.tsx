...
export const RunsTable = () => {
  ...
  const [failedRetryRuns, setFailedRetryRuns] = useState<
    Required<ExecutionWithRetryError>[]
  >([]);
  const [failedRetryDialogOpen, setFailedRetryDialogOpen] = useState(false);
  const [errorDialogRun, setErrorDialogRun] = useState<Execution | null>(null);
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['execution-table', searchParams.toString(), workspaceId],
  ...
  const filters: DataTableFilters<keyof Execution | 'failedStepMessage'>[] =
    useMemo(
      () => [
        {
          type: 'select',
  ...
  const cancelRuns = executionMutations.useBulkCancelRuns({
    onSuccess: () => {
      refetch();
      setSelectedRows([]);
  ...
  const archiveRuns = executionMutations.useBulkArchiveRuns({
    onSuccess: () => {
      refetch();
    },
  });

  const bulkActions: BulkAction<Execution>[] = useMemo(
    () => [
      {
      ...
      },
      {
        render: (_, resetSelection) => {
          const allCancellable = selectedRows.every(
            (row) =>
              row.status === ExecutionStatus.PAUSED ||
              row.status === ExecutionStatus.QUEUED,
          );
          const isDisabled =
            selectedRows.length === 0 ||
                          ...
                          status.length > 0
                            ? (status.filter(
                                (s) =>
                                  ...
                                  s === ExecutionStatus.QUEUED,
                              ) as (
                                | typeof ExecutionStatus.PAUSED
  ...
  const handleRowClick = useCallback(
    (row: Execution, newWindow: boolean) => {
      if (newWindow) {
        openNewWindow(
