...
A **waitpoint** is the durable row that represents a paused step on a workflow run. The workflow run row only carries status (`PAUSED`, `RUNNING`, …); the *why* lives on the waitpoint.

```mermaid
stateDiagram-v2
    [*] --> PENDING: createWaitpoint
    PENDING --> COMPLETED: resume signal
    COMPLETED --> [*]: workflow resumes
...
| Field | Meaning |
| --- | --- |
| `executionId`, `stepName` | Which run/step is paused. Unique together: a step has at most one waitpoint. |
...
| `resumePayload` | `{ body, headers, queryParams }` from the resume call, surfaced to the connector as `ctx.resumePayload`. |
...
1. **Create.** The connector calls `ctx.run.createWaitpoint({ type, ... })` + `ctx.run.waitForWaitpoint(id)`. The engine marks the step as `paused` and asks the server to insert a `PENDING` row. The insert is idempotent on the `(workflow run, step)` pair.
2. **Checkpoint.** The engine serializes the execution context and transitions the workflow run to `PAUSED`.
...
sequenceDiagram
    ...
    Connector->>Engine: createWaitpoint + waitForWaitpoint
    Engine->>Server: register waitpoint
    Server-->>Engine: { id, resumeUrl }
    Engine->>Engine: checkpoint context, status = PAUSED
    Note over Connector,Server: ...time passes...
    Caller->>Server: HTTP call on resumeUrl
    Server->>Queue: enqueue resume job
    Queue->>Engine: resume
    Engine->>Connector: re-invoke with resumePayload
```

## Resume-before-pause race

A callback can arrive before the workflow run has finished writing `PAUSED` to disk. The protocol absorbs the race:
...
- When the workflow run transitions to `PAUSED`, the server checks for a matching `COMPLETED` waitpoint and enqueues the resume job immediately.
...
| `ALL` | `/v1/executions/:id/waitpoints/:waitpointId` | Resume, async. |
| `ALL` | `/v1/executions/:id/waitpoints/:waitpointId/sync` | Resume, sync. The HTTP response is whatever the workflow produces after resuming. |

## Connector API

Connector authors create waitpoints with `ctx.run.createWaitpoint` / `ctx.run.waitForWaitpoint`. Patterns for `WEBHOOK`, `DELAY`, and `responseToSend` are in Workflow Control.
