...
The `ctx` parameter inside triggers and actions provides a simple key/value storage mechanism. The storage is persistent, meaning that the stored values are retained even after the execution of the connector.

By default, the storage operates at the workflow level, but it can also be configured to store values at the project level.
