...
- `mcpServerService.buildServer()` — builds the server per-request: metadata → dynamic workflow tools → controllable + locked static tools → empty resources/prompts (spec compliance).
...
- AI connectors consume MCP tools over three transports: `SIMPLE_HTTP`, `STREAMABLE_HTTP`, `SSE`.
- Embed SDK adds `authorizeMcp()` (in-embed OAuth consent), `mcpSettings()`, and `generateMcpToken()` (mints `{ mcpServerUrl, mcpToken }` with no OAuth workflow, backed by `POST /v1/projects/:projectId/mcp-server/token` — a short-lived 15-min project-scoped token).
...
- `packages/server/api/src/app/mcp/tools/` — locked and controllable tool definitions, plus curated connector expertise notes
- `packages/server/api/src/app/mcp/oauth/` — OAuth 2.0 PKCE workflow: metadata, authorize, token, revoke
- `packages/core/shared/src/lib/automation/mcp/` — McpServer schema, McpToolDefinition, MCP OAuth types
- `packages/web/src/app/components/project-settings/mcp-server/` — settings panel: credentials, workflows-as-tools, tool toggles
