...
description: "Configure SCIM provisioning from Okta to FEMA Integration Platform."
---
...
- Generated an API key from `/platform/security/api-keys` route in the FEMA Integration Platform app

## Configure SCIM Connection in Okta

<Steps>
  <Step title="Open your Okta app">
    In Okta Admin Console, open your FEMA Integration Platform application (created in first [SSO step](/admin-guide/guides/sso#saml-with-okta)).

  </Step>
  <Step title="Enable provisioning">
    In the app's **General** tab, enable **SCIM Provisioning**.
    ![Okta User Provisioning](/resources/screenshots/scim/okta/enable-provisioning.png)
  </Step>
  <Step title="Add SCIM connection details">
    **Provisioning** will be visible, go to it and set:
    - **SCIM base URL** to `https://your-fema-domain/api/v1/scim/v2`
...
| FEMA Integration Platform (SCIM) | Okta Value |
| --- | --- |
| `userName` | `user.email` |
...
<Steps>
  ...
  <Step>
    ...
    | `External namespace` | `urn:ietf:params:scim:schemas:fema:1.0:CustomUserAttributes` |
    | `Enum` | `enabled` |
  ...
  <Step>
    ...
    <Warning>
      This step assumes that you already have a field in the Okta user profile that you can map to platformRole in your FEMA Integration Platform user profile. If you don't have one, you can create a new field in **Directory -> Profile Editor -> User (default)**.
    </Warning>

     - Back to your FEMA Integration Platform application page in **Provisioning -> To App -> Attribute Mappings**
...
In the FEMA Integration Platform application page, go to **Provisioning -> To App** and enable the actions you want to be applied to FEMA Integration Platform when changes occur in Okta.

![Okta role expression example](/resources/screenshots/scim/okta/provision-actions.png)

Now in the **Assignments** tab you can:
- Choose to provision individual users or groups. Note that groups in Okta will be projects in FEMA Integration Platform.

- In case you don't have groups and you want to provision your Okta users at once, you can assign the `Everyone` group.

- When editing/creating users in an assigned group (including `Everyone`), they should be updated in FEMA Integration Platform.

- To push groups to FEMA Integration Platform, go to the **Push groups** tab and click on the push button, find the group and save.
...
<Tip>
  Created users in FEMA Integration Platform will receive a welcome email. When clicked, they will be redirected to sign in with `SAML`.
</Tip>


### Deprovision

Users' state switches to `INACTIVE` in FEMA Integration Platform only when they are deactivated in Okta. Suspension or deletion in Okta does not reflect in FEMA Integration Platform because of Okta's design.
...
<Warning>
  Deleting a group will delete the whole project in FEMA Integration Platform with its workflows and connections. Users linked to that group won't be affected.
</Warning>
