...
Without it, a worker that dies mid-workflow would restart the whole run from the trigger and re-send emails, re-charge cards, and re-call APIs. Instead, a fresh worker reuses the saved output of every finished step and runs only the first step that had not completed. The same mechanism covers crashes, deploys, long pauses, and retries.

## The run log

Every workflow run has a **run log**: one compressed checkpoint file with everything needed to resume the run on a fresh worker.
...
Resume is not a special path. Each time a worker starts a run, it walks the workflow graph from the trigger and asks at every step: is this step's output already in the log?
...
- **Paused step.** The connector creates a [waitpoint](/install/architecture/waitpoints). When the waitpoint fires, a resume job is queued and a worker replays the run.
