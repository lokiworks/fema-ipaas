  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        {t(
          'Allocate capacity per project — pick its worker group and concurrency limit. Leave the limit empty to inherit the tenant default. Project settings shows these read-only and links here.',
        )}
      </p>
...
function useTenantProjectsPage({
  cursor,
  limit,
  displayName,
}: {
  ...
  return useQuery<SeekPage<ProjectWithLimits>>({
    queryKey: ['tenant-projects-page', cursor, limit, displayName],
    queryFn: () =>
...
async function saveProjectUpdate({
  ...
  projectId: string;
  update: UpdateProjectTenantRequest;
