...
# Gotcha: the code cache is namespaced by `workflowVersionId` — never hand it a constant

`workflowVersionId` is the **only** namespace separating one compiled code step from another, and it does
...
- Build path: `codeCache.stepDir({ workflowVersionId, stepName })` → `<codeCache>/<workflowVersionId>/<stepName>/index.js`
  (`sandbox/cache/workflow/code/code-cache.ts`).
- Read path: the engine loads `${FEMA_BASE_CODE_DIRECTORY}/${constants.workflowVersionId}/${action.name}/index.js`
  (`engine/handler/code-executor.ts`) — same two segments, no other keying.

Two things break when the id is a shared constant (e.g. `DEFAULT_MCP_DATA.workflowVersionId`) and the step
name is fixed:
...
1. **Cross-tenant code execution.** A hash mismatch `rm -rf`s the directory and rebuilds. `memoryLock`
   ...
   `FEMA_WORKER_CONCURRENCY=5` boxes sharing one on-disk cache, run A can end up executing run B's
...
2. **No mount in isolate mode.** `buildCodeMount` returns `null` when `reusable === false` and
   `workflowVersionId` is nil, so nothing lands at `/root/codes` and the artifact is invisible inside the
...
Workflow runs are safe by construction — a real `workflowVersion.id`. The trap is any workflowless execution path
...
result on `provision.workflowVersionId` (so the mount is built) *and* on the `CodeArtifact` *and* on
`EngineConstants.workflowVersionId` (what the engine reads). All three must be the same string. It yields
...
a live dir unreachable. The touch is gated on `isManagedDir`, so workflow-version dirs pay nothing for it.
...
it feeds; pinned by the concurrency case in `sandbox/test/lib/cache/workflow/code/code-builder.test.ts` and, end
to end, by `handles concurrent workflow run executions without jobs getting stuck` in
`api/test/integration/ce/workflows/execution/execute-workflow-e2e.test.ts`.
