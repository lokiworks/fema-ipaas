...
import { bundleConnectorUtils } from './bundle-connector-utils'
...
describe('bundleConnector — external dependency capture', () => {
    ...
    it('captures a native dep reached transitively through an inlined package', async () => {
        ...
        const connectorPath = join(root, 'connector')
        mkdirSync(join(connectorPath, 'src'), { recursive: true })
        writeFileSync(join(connectorPath, 'package.json'), JSON.stringify({ name: 'connector-x', version: '0.0.1', dependencies: { 'fake-sdk': '1.0.0' } }))
        writeFileSync(join(connectorPath, 'src', 'index.ts'), 'import { db } from \'fake-sdk\'\nexport const connector = { db }\n')
        const distPath = join(connectorPath, 'dist')
        mkdirSync(distPath, { recursive: true })

        const result = await bundleConnectorUtils.bundleConnector({ connectorPath, distPath, repoRoot: root })
    ...
    it('keeps wasm/native asset packages (tiktoken, sharp) external so their runtime-loaded assets resolve', async () => {
        ...
        const connectorPath = join(root, 'connector')
        mkdirSync(join(connectorPath, 'src'), { recursive: true })
        writeFileSync(join(connectorPath, 'package.json'), JSON.stringify({ name: 'connector-z', version: '0.0.1', dependencies: { tiktoken: '1.0.11', sharp: '0.35.2' } }))
        writeFileSync(join(connectorPath, 'src', 'index.ts'), 'import * as tiktoken from \'tiktoken\'\nimport * as sharp from \'sharp\'\nexport const connector = { tiktoken, sharp }\n')
        const distPath = join(connectorPath, 'dist')
        mkdirSync(distPath, { recursive: true })
    ...
    it('externalizes an inlined package that relies on import.meta', async () => {
        ...
        const connectorPath = join(root, 'connector')
        mkdirSync(join(connectorPath, 'src'), { recursive: true })
        writeFileSync(join(connectorPath, 'package.json'), JSON.stringify({ name: 'connector-y', version: '0.0.1', dependencies: { 'esm-dep': '1.0.0' } }))
        writeFileSync(join(connectorPath, 'src', 'index.ts'), 'import { req } from \'esm-dep\'\nexport const connector = { req }\n')
        const distPath = join(connectorPath, 'dist')
        mkdirSync(distPath, { recursive: true })
...
describe('bundleConnector — forked sibling entries (GIT-1772)', () => {
    let root: string | undefined
    ...
    function setupForkingConnector({ declareEntry }: { declareEntry: boolean }): string {
        root = mkdtempSync(join(tmpdir(), 'ap-bundle-'))
        ...
        const connectorPath = join(root, 'connector')
        mkdirSync(join(connectorPath, 'src', 'lib'), { recursive: true })
        writeFileSync(join(connectorPath, 'package.json'), JSON.stringify({
            name: 'connector-forking',
            version: '0.0.1',
            dependencies: { oracledb: '6.10.0' },
            ...(declareEntry ? { bundleForkedEntries: ['src/lib/runner.ts'] } : {}),
        }))
        writeFileSync(join(connectorPath, 'src', 'index.ts'), 'import { start } from \'./lib/pool\'\nexport const connector = { start }\n')
        writeFileSync(join(connectorPath, 'src', 'lib', 'pool.ts'), 'import { fork } from \'child_process\'\nimport { join } from \'path\'\nexport const start = () => fork(join(__dirname, \'runner.js\'))\n')
        writeFileSync(join(connectorPath, 'src', 'lib', 'protocol.ts'), 'export const READY_MESSAGE = \'runner-ready\'\n')
        writeFileSync(join(connectorPath, 'src', 'lib', 'runner.ts'), 'import db from \'oracledb\'\nimport { READY_MESSAGE } from \'./protocol\'\nconsole.log(READY_MESSAGE, db)\n')
        mkdirSync(join(connectorPath, 'dist'), { recursive: true })
        return connectorPath
    }

    it('emits a declared forked entry beside the main bundle and keeps its native dep external', async () => {
        const connectorPath = setupForkingConnector({ declareEntry: true })

        const result = await bundleConnectorUtils.bundleConnector({ connectorPath, distPath: join(connectorPath, 'dist'), repoRoot: root! })

        expect(result.extraBundleFiles).toEqual(['src/runner.js'])
        const runnerBundle = readFileSync(join(connectorPath, 'dist', 'src', 'runner.js'), 'utf-8')
    ...
    it('fails loudly when connector source uses __dirname without declaring a forked entry', async () => {
        const connectorPath = setupForkingConnector({ declareEntry: false })

        await expect(bundleConnectorUtils.bundleConnector({ connectorPath, distPath: join(connectorPath, 'dist'), repoRoot: root! }))
            .rejects.toThrow(/bundleForkedEntries/)
    })

    it('fails loudly on a declared entry that does not exist', async () => {
        const connectorPath = setupForkingConnector({ declareEntry: true })
        rmSync(join(connectorPath, 'src', 'lib', 'runner.ts'))
    ...
    it('fails loudly when two declared entries collide on the same output name', async () => {
        const connectorPath = setupForkingConnector({ declareEntry: true })
        mkdirSync(join(connectorPath, 'src', 'lib', 'other'), { recursive: true })
        writeFileSync(join(connectorPath, 'src', 'lib', 'other', 'runner.ts'), 'export const other = true\n')
        writeFileSync(join(connectorPath, 'package.json'), JSON.stringify({
            name: 'connector-forking',
            version: '0.0.1',
            dependencies: { oracledb: '6.10.0' },
