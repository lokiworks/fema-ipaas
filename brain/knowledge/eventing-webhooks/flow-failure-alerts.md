...
### How it works
- `sendAlertOnRunFinish({ issueToAlert, executionId, failedStep })` runs after a failed production run. It increments Redis `flow_fail_count:<flowVersionId>` (expires 86400s); only when the count is 1 does it email.
...
### Key files
Entry point: `alertsService`, called from `packages/server/api/src/app/flows/execution/execution-hooks.ts` when a run finishes.
