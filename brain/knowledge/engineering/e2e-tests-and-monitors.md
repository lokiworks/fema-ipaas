...
- **Name the list by pattern, not by package.** The list held `@fema-ipaas/connector-sdk` alone, so the tests of 13 other connector packages — csv, http, text-helper, date-helper, webhook, crypto, xml, subflows, math-helper, data-mapper, data-summarizer, file-helper, schedule, ~275 tests — had never run in CI. Widening the entry to `--filter='@fema-ipaas/connector-*'` picked all of them up (turbo skips packages with no `test` script, so it stays correct as connectors come and go) and immediately surfaced two things nothing else would have: `connector-store` did not compile at all (it still referenced `StoreScope.WORKSPACE`, renamed to `PROJECT` — the package was in no build path either), and `connector-http` had three tests asserting env-var proxy wiring that `FetchHttpClient` deliberately dropped, which fell through to a real DNS lookup of `api.example.com`. Quote the glob so the shell does not expand it.
