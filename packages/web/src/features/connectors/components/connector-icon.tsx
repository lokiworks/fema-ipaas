...
const connectorIconVariantsWithPadding = cva('', {
  variants: {
    size: {
      xxl: 'p-4',
      xl: 'p-3',
...
interface ConnectorIconProps
  extends VariantProps<typeof connectorIconVariants> {
  displayName?: string;
  logoUrl?: string;
  showTooltip: boolean;
  background?: string;
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            className={cn(
              connectorIconVariants({ border, size }),
              'overflow-hidden',
            )}
          ...
          >
            {logoUrl ? (
              <ImageWithColorBackground
                ...
                alt={displayName}
                className={cn(
                  connectorIconVariantsWithPadding({ size }),
