...
function copyPackageJson({ connectorPath, distPath }: ConnectorDistPaths): void {
    const srcPackageJson = join(connectorPath, 'package.json')
...
function copyI18nAssets({ connectorPath, distPath }: ConnectorDistPaths): void {
    const i18nSrc = join(connectorPath, 'src', 'i18n')
...
async function prepareConnectorDistForPublish(connectorPath: string): Promise<void> {
    const distPath = join(connectorPath, 'dist')

    if (!existsSync(distPath)) {
        throw new Error(`[prepareConnector] no dist output at ${distPath} for ${connectorPath}`)
    }

    const repoRoot = findRepoRoot(connectorPath)
    const paths = { connectorPath, distPath }
    copyPackageJson(paths)
    copyI18nAssets(paths)

    const { bundleBytes, rawBytes, external, extraBundleFiles } = await bundleConnectorUtils.bundleConnector({ ...paths, repoRoot })
...
function rewriteManifestForBundle({ distPath, external, repoRoot, extraBundleFiles = [] }: { distPath: string, external: string[], repoRoot: string, extraBundleFiles?: string[] }): void {
    ...
    for (const dep of external) {
        if (bundleConnectorUtils.OPTIONAL_EXTERNALS.has(dep)) {
            continue
        }
    ...
    json.main = `./${bundleConnectorUtils.BUNDLE_FILENAME}`
    ...
    json.files = [bundleConnectorUtils.BUNDLE_FILENAME, ...extraBundleFiles, 'package.json', 'src/i18n']
...
export { prepareConnectorDistForPublish, resolveInstalledVersion, findInstalledVersion, rewriteManifestForBundle }

type ConnectorDistPaths = {
