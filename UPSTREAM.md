...
本仓库是 Activepieces Community Edition 的二次开发分支。本文件按设计文档 §46 Step 0
...
| `LICENSE_BASELINE` | MIT（根 LICENSE）+ Activepieces Enterprise License（`packages/ee/`、`packages/server/api/src/app/ee/`） |
...
- `packages/ee/` 与 `packages/server/api/src/app/ee/` 下的全部内容，适用
  `packages/ee/LICENSE` 定义的 **Activepieces Enterprise License**（商业许可，不可自由使用）。
- 其余内容适用 **MIT**。

因此本项目在 Step 1 中**完整删除**上述两个目录，且不得从其中复制任何代码片段到
MIT 区域。删除完成后，本仓库的全部继承代码来源于上游 MIT 部分。

第三方连接器（原 `packages/pieces/community/*`）各自可能携带独立许可证，按 §45.4
