...
### How it works
- Builds a `webhook → data-mapper → return-response` workflow. Instead of a raw `--concurrency`, it **auto-discovers the deployment shape** (`GET /v1/worker-machines`) and drives load = the effective **execution slot** count, so a healthy deploy queues ~zero by construction. Any queue-wait it reports is a real finding (usually driven concurrency > slots).
...
### Gotchas
- Auth is **platform API key only** (`FEMA_API_KEY`/`--api-key` + `--project-id`) — email/password login was removed; SERVICE principal gets the full diagnostic bundle.
- The infra round-trip block is **self-hosted only** — `/v1/health/diagnostics` returns `FEATURE_DISABLED` on `FEMA_EDITION=cloud` (a Cloud admin is a tenant, not the infra operator); the CLI degrades gracefully.
...
- **Never publish numbers from two cluster shapes on the same page.** The docs once carried `93.5 / 148.9 req/s` (Experiment 1, `e2-standard-4` × 14) *and* `185.3 / 409.5 req/s` (a later `n2-standard-16` × 10 run) for the same `4 app · 40 w` / `8 app · 80 w` rows. Same ratio, same workflow, different hardware — so the two disagreed by 2–3×. Every number on the page must come from one rig, and the diagram + `changelog.mdx` + `latency.mdx` cross-references have to move with it.
...
- `packages/cli/src/lib/commands/benchmark.ts` — the whole command: auth, workflow build, discovery, load, attribution, reporting. Its spec sits next to it.
