...
const WorkflowDragLayer = ({ children }: { children: React.ReactNode }) => {
  ...
  const draggedStep = activeDraggingStep
    ? workflowStructureUtil.getStep(activeDraggingStep, workflowVersion.trigger)
    : undefined;
  const handleDragStart = (e: DragStartEvent) => {
    if (e.active.data.current?.type === workflowCanvasConsts.DRAGGED_STEP_TAG) {
      setActiveDraggingStep(e.active.id.toString());
    }
    if (e.active.data.current?.type === workflowCanvasConsts.DRAGGED_NOTE_TAG) {
...
function handleStepDragEnd({
  ...
  BuilderState,
  'applyOperation' | 'activeDraggingStep' | 'workflowVersion'
>) {
  const draggedStep = activeDraggingStep
    ? workflowStructureUtil.getStep(activeDraggingStep, workflowVersion.trigger)
    : undefined;
  ...
  if (isOverSomething) {
    ...
    if (
      ...
      const isPartOfInnerWorkflow = workflowStructureUtil.isChildOf(
        draggedStep,
        droppedAtNodeData.parentStepName,
      );
      if (isPartOfInnerWorkflow) {
