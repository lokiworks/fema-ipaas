...
Every FEMA Integration Platform project can act as an [MCP](/mcp/overview) server — a place an AI can connect to in order to run that project's workflows.

When you embed FEMA Integration Platform, your users log in through **your** app, not FEMA Integration Platform. So they can't use the normal "connect your AI" screen (it would ask them to log in to FEMA Integration Platform, which they can't).
...
- **FEMA Integration Platform** checks everything and gives back a token for that one user's project.

<Info>
It's normal OAuth. The only FEMA Integration Platform-specific part is one SDK call (`authorizeMcp`) that shows the Authorize popup inside your app.
</Info>

## Quick token (no OAuth)

If you don't need the full OAuth workflow — you just want a token to point an AI at the embedded user's project — call `generateMcpToken()` from the frontend. It uses the embed session you already configured, so there's no app registration, no PKCE, and no popup.

```ts
const { mcpServerUrl, mcpToken } = await fema.generateMcpToken();
...
<Note>
Use this when your app holds the MCP client and the embedded user is already signed in through the SDK. Use the **OAuth workflow below** when a separate third-party app needs long-lived, revocable access on the user's behalf (it returns a refresh token).
</Note>
...
3. Your frontend called `fema.configure({ jwtToken, instanceUrl, … })`.

Below, `INSTANCE_URL` is your FEMA Integration Platform address (like `https://app.your-company.com`).

## The whole idea in 6 steps

1. **Register your app once** → you get a `client_id`.
2. **Make two random codes** (a "verifier" and a "challenge").
3. **Ask FEMA Integration Platform to start a connection** → you get an `authRequestId`.
4. **Show the Authorize popup** in your app → the user clicks Authorize → you get a `code`.
5. **Trade the `code` for a token.**
6. **Use the token** to run the user's workflows.

## Steps

<Steps>
  <Step title="1. Register your app (only once)">
    This tells FEMA Integration Platform who your app is. Save the `client_id` — it's the same for all users.
  ...
  <Step title="4. Show the Authorize popup">
    ...
    ```ts
    const result = await fema.authorizeMcp({ authRequestId });
  ...
  <Step title="6. Use the token">
    ...
    You'll see the user's tools and workflows come back.
  </Step>
</Steps>
