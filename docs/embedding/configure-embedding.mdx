...
<Steps>
  <Step title="Step 1: Enter the embed URL (Cloud only)">
    Pick the subdomain you'll embed in your website (e.g. `workflows.acme.com`). This URL will be visible inside workflows and is the origin the iframe loads from.
  ...
  <Step title="Step 3: Add allowed websites">
    ...
    <Tip>
      You can also pre-seed origins via the `FEMA_ALLOWED_EMBED_ORIGINS` environment variable on self-hosted instances. Origins set this way are merged with the ones configured in the UI.
    </Tip>
  </Step>
