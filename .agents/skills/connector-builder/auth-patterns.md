...
**Real example:** `packages/connectors/community/stripe/src/index.ts`
...
**Real example:** `packages/connectors/community/github/src/index.ts`, `packages/connectors/community/zoho-campaigns/` (OAuth2 with extra `props`)
...
**Real example:** `packages/connectors/community/wordpress/src/index.ts`, `packages/connectors/community/mattermost/src/index.ts`
...
Add a `refresh` field to cache the token server-side. FEMA Integration Platform renews it automatically up to 15 minutes before expiry (clamped to half the token lifetime for short-lived tokens).
...
export const myAppAuth = ConnectorAuth.CustomAuth({
  ...
  props: {
    ...
    password: ConnectorAuth.SecretText({ displayName: 'Password', required: true }),
  },
  validate: async ({ auth }) => {
    // validate as usual
  },
...
**Real example:** `packages/connectors/community/umami/src/lib/auth.ts`
...
- For `OAuth2`, FEMA Integration Platform already derives an identifier from the token response's OIDC claims when present. Add this hook only when the provider needs handling the generic path can't cover (e.g. Slack has no per-user OIDC claim, so it needs a workspace name plus a follow-up `users.info` call).
...
**Real example:** `packages/connectors/community/slack/src/lib/auth.ts`

---

## No Auth

For public APIs or utility connectors that don't need credentials.
...
**Real example:** `packages/connectors/core/qrcode/src/index.ts`
