...
icon: "wrench"
description: "Complete catalog of all MCP tools available in FEMA Integration Platform"
---
...
List workflows in the current project with status, trigger type, and published state.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `limit` | number | No | Max workflows to return (default 100, max 500) |
| `status` | string | No | Filter by status: `ENABLED` or `DISABLED` |
| `name` | string | No | Filter by workflow name (partial match) |

### ap_workflow_structure

Get the full structure of a workflow: step tree, configuration status, step input values, router branch conditions, and valid insert locations. Shows the actual configured inputs for each step (URL, body, headers, etc.) so the AI can read and edit existing configurations.
...
Read the full source code, package.json, and input mappings of a CODE step. Returns untruncated content — unlike `ap_workflow_structure` which truncates code to 300 characters.
...
<Tip>
Use this tool when you need to read or edit a CODE step's full source. `ap_workflow_structure` truncates code for overview purposes, so always call `ap_read_step_code` before modifying a code step.
</Tip>

### ap_read_step_settings

Read the full untruncated settings of any step, including the trigger: connector input, action/trigger name, loop items, router branches, and error handling options. Use this to see a step's current configuration before updating it — `ap_workflow_structure` truncates connector input.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowId` | string | Yes | The workflow ID |
| `stepName` | string | Yes | The step name (e.g., `trigger`, `step_1`). Use `ap_workflow_structure` to get valid values. |

### ap_validate_workflow

Validate a workflow for structural issues without publishing. Checks step validity, template references, and empty branches.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowId` | string | Yes | The workflow ID |

### ap_research_connectors

Research available connectors. Use `connectorNames` for bulk exact lookup (always returns actions and triggers). Use `searchQuery` for fuzzy discovery.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `connectorNames` | string[] | No | Exact connector names to look up (always returns actions and triggers) |
| `searchQuery` | string | No | Filter connectors by name |
...
| `connectorName` | string | No | Restrict results to a single connector (e.g. `slack`). Omit to search the whole catalog. |

<Tip>
This is the discovery step of the run-action workflow: take a result's `connectorName` + `actionName` to `ap_get_connector_props` for the input schema, then execute with `ap_run_action`.
</Tip>

### ap_search_triggers

Find connector triggers (the event that starts a workflow) by describing when the workflow should run (e.g. *"when a new row is added to a Google Sheet"*). Same behavior and result shape as `ap_search_actions`, returning trigger names instead of action names.

Available when [tool search](/mcp/tool-search) is enabled on the instance.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `query` | string | Yes | Natural-language description of the event that should start the workflow |
| `limit` | number | No | Max matches to return (default 5, max 20) |
| `connectorName` | string | No | Restrict results to a single connector. Omit to search the whole catalog. |

### ap_get_connector_props

Get the detailed input property schema for a specific connector action or trigger. Returns field names, types, required/optional, descriptions, default values, and dropdown options. When auth is required but not provided, automatically lists available connections. Use this before `ap_update_step` or `ap_update_trigger` to know exactly which fields to set.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `connectorName` | string | Yes | Connector name (e.g., `@fema-ipaas/connector-slack`) |
...
| `workflowId` | string | No | Workflow ID for resolving dependent dropdowns that need step context. |
...
Resolve dropdown options for a single connector property. Returns available choices with labels and internal IDs. Use this to discover valid values for dynamic dropdown fields (e.g., Slack channels, Google Sheets, email labels) before configuring a step.
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `connectorName` | string | Yes | Connector name (e.g., `@fema-ipaas/connector-google-sheets`) |
...
Validate a step configuration before applying it. Returns field-level errors without modifying any workflow.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `stepType` | string | Yes | `CONNECTOR_ACTION`, `CONNECTOR_TRIGGER`, `CODE`, `LOOP_ON_ITEMS`, or `ROUTER` |
| `connectorName` | string | No | For CONNECTOR types: connector name (short names like `slack` accepted) |
| `actionName` | string | No | For CONNECTOR_ACTION: action name |
| `triggerName` | string | No | For CONNECTOR_TRIGGER: trigger name |
| `input` | object | No | For CONNECTOR types: input config to validate |
| `auth` | string | No | For CONNECTOR types: any non-empty string indicates auth is provided |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `connectorName` | string | No | Filter by connector name. Short names like `slack` or `google-sheets` are auto-expanded. |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `provider` | string | No | Filter by provider (`openai`, `anthropic`, `google`, `azure`, `openrouter`, `fema`, `cloudflare-gateway`, `custom`) |
...
Get detailed results of a workflow run including step-by-step outputs, errors, and durations.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `executionId` | string | Yes | The run ID |
...
| `connectorName` | string | No | For connections: which connector needs auth |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowName` | string | Yes | Display name for the workflow |

### ap_duplicate_workflow

Duplicate an existing workflow. Creates a new copy with all steps, configuration, and canvas notes. Connections and sample data are not copied.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowId` | string | Yes | The workflow ID to duplicate |
| `name` | string | No | Name for the copy (defaults to "Copy of \{original name\}") |

<Tip>
After duplicating, use `ap_workflow_structure` on the new workflow to check configuration status. Steps that reference connections will need to be re-configured with `ap_update_step`.
</Tip>
...
Permanently delete a workflow and all its versions. This cannot be undone.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowId` | string | Yes | The ID of the workflow to delete |

### ap_lock_and_publish

Publish the current draft of a workflow. Validates all steps are configured.
...
Add, configure, and remove steps in a workflow.

### ap_build_workflow

Create a complete workflow in one call — trigger plus any number of steps. Steps are added sequentially (trigger → step_1 → step_2 → ...). All steps are validated on creation. Use granular tools (`ap_add_step`, `ap_update_step`) to modify existing workflows or add nested structures (loop contents, router branches).

Returns the workflow ID, a direct `workflowUrl` link to the workflow in the editor, step count, and validation status.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowName` | string | Yes | Name for the new workflow |
| `trigger` | object | Yes | `{connectorName, triggerName, input?, auth?}` |
| `steps` | array | Yes | Array of step specs, each with `type`, `displayName`, and type-specific fields |

**Step types in the array:**
- **CONNECTOR**: `connectorName`, `actionName`, `input`, `auth`, `continueOnFailure`, `retryOnFailure`
...
Set or update the trigger for a workflow.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `workflowId` | string | Yes | The workflow ID |
| `connectorName` | string | Yes | Connector name (e.g., `@fema-ipaas/connector-gmail`) |
| `triggerName` | string | Yes | Trigger name from the connector |
...
Add a new step to a workflow. Optionally configure it in the same call by providing `input`, `auth`, `sourceCode`, or `loopItems`.
...
| `stepType` | string | Yes | `CODE`, `CONNECTOR`, `LOOP_ON_ITEMS`, or `ROUTER` |
| `displayName` | string | Yes | Step display name |
| `connectorName` | string | No | For CONNECTOR steps |
| `actionName` | string | No | For CONNECTOR steps |
...
| `continueOnFailure` | boolean | No | For CODE/CONNECTOR steps: continue workflow if this step fails (default: false) |
| `retryOnFailure` | boolean | No | For CODE/CONNECTOR steps: retry this step on failure (default: false) |
...
| `continueOnFailure` | boolean | No | For CODE/CONNECTOR steps: continue workflow if this step fails |
| `retryOnFailure` | boolean | No | For CODE/CONNECTOR steps: retry this step on failure |
...
Manage conditional branches in router steps. Use `ap_workflow_structure` to see existing branch conditions and indices.
...
Add, update, or delete canvas notes on a workflow.
...
Test workflows, inspect results, and retry failures. Test tools poll for up to 120 seconds and return step-by-step results.

### ap_test_workflow

Test a workflow end-to-end in the test environment. Pass `triggerTestData` to provide mock trigger output when no sample data exists (e.g., for webhook triggers).
...
<Warning>
The workflow must have a configured trigger. The tool validates this before running and returns a clear error if not.
</Warning>

### ap_test_step

Test a single step within a workflow. Runs all steps up to and including the target step. Pass `triggerTestData` when no sample data exists.
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `executionId` | string | Yes | The failed run ID |
...
- **ON_LATEST_VERSION**: Re-run the entire workflow with the current published version

### ap_run_action

Execute a single connector action once without building or saving a workflow. Designed for one-shot tasks like *"check my inbox"* or *"send one Slack message"* where building a full automation would be overkill. Under the hood the action runs directly in the engine and the result is returned immediately — no workflow is created, and nothing appears in the user's workflow list.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `connectorName` | string | Yes | Connector name (e.g. `slack` or `@fema-ipaas/connector-slack`). Use `ap_research_connectors` to discover. |
| `actionName` | string | Yes | Action to run (e.g. `send_channel_message`). Use `ap_get_connector_props` for the input shape. |
| `input` | object | No | Fully-resolved input for the action. Keys must match the connector action's props. Pass raw values — do **not** wrap them in `{{…}}`. Omit entirely if the action has no props. |
| `connectionExternalId` | string | No | `externalId` from `ap_list_connections`. Required if the connector needs auth. Auto-wrapped server-side as `{{connections['externalId']}}`. Must be a plain ID — special characters are rejected. |

<Info>
When to pick this vs. `ap_build_workflow`:
- **`ap_run_action`** — one-off, throwaway, returns a result now.
- **`ap_build_workflow`** — persistent automation that should repeat, run on a schedule, or trigger on external events.
</Info>

Prefer calling `ap_list_connections` and `ap_get_connector_props` first so you know the exact `actionName`, the expected props, and the right `connectionExternalId` before invoking. Missing required inputs or unknown actions produce a friendly error before dispatch — no run is created and nothing is billed.
