...
export const projectController: FastifyPluginAsyncZod = async (app) => {
    ...
    app.post('/:id', UpdateProjectRequest, async (request) => {
        const project = await assertProjectBelongsToTenant({
...
async function assertProjectBelongsToTenant({ projectId, tenantId, log }: AssertParams) {
    const project = await projectService(log).getOneOrThrow(projectId)
    assertNotNullOrUndefined(project.tenantId, 'tenantId')
...
const ListProjectsRequest = {
    ...
    schema: {
        tags: ['projects'],
        description: 'List the projects the caller can access inside the current tenant.',
        security: [SERVICE_KEY_SECURITY_OPENAPI],
        querystring: ListProjectRequestForTenantQueryParams,
        response: {
...
const CreateProjectRequest = {
    ...
    schema: {
        tags: ['projects'],
        description: 'Create a project inside the current tenant.',
        security: [SERVICE_KEY_SECURITY_OPENAPI],
...
const UpdateProjectRequest = {
    ...
    schema: {
        tags: ['projects'],
        description: 'Update a project inside the current tenant.',
        security: [SERVICE_KEY_SECURITY_OPENAPI],
        params: z.object({ id: z.string() }),
        body: UpdateProjectTenantRequest,
        response: {
