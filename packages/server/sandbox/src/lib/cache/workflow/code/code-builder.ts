...
import { tryCatch, tryCatchSync } from '@fema/core-utils'
import { type ApLogger, cryptoUtils, fileSystemUtils, wideEvent } from '@fema/server-utils'
import { ExecutionMode } from '@fema/shared'
...
export const codeBuilder = (log: ApLogger, getSettings: () => SandboxSettings) => ({
    async processCodeStep({
        ...
        const { sourceCode, workflowVersionId, name } = artifact
        const codePath = codeCache(codesFolderPath).stepDir({ workflowVersionId, stepName: name })
        log.debug({ sourceCode, name, codePath }, 'Processing code step')

        const currentHash = await cryptoUtils.hashObject(sourceCode)
        const compiledStepPath = codeCache(codesFolderPath).compiledStepPath({ workflowVersionId, stepName: name })
