...
- `FEMA_ENFORCE_CONNECTION_PIECE_BINDING` (default `false`) makes a step resolve only connections whose `pieceName` equals the step's own piece; a mismatch raises a USER-level `ConnectionPieceMismatchError`. The check lives in the **engine's** `connection-resolver`, not the worker endpoint. Set the var on the **app** container — the engine cannot read `process.env` (sandbox env is an allowlist), so the flag rides `WorkerSettings` → `SandboxSettings` → sandbox env, the same path as `FEMA_DEV_PIECES`. Code / loop / router steps have no piece, so a missing name is a denial — they lose connection access entirely, and enabling the flag breaks flows that feed a connection into custom JS.
