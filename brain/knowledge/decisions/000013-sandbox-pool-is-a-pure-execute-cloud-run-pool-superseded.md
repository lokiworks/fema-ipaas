...
## Decision
Superseded by "Worker is the Sandbox" for the hosting model. The sandbox pool is a pure `execute()` that holds no `apiClient` and no app connection; every input is passed in as request/response data (flow bundle + connectors as S3 refs). The worker is the sole Resolver, materializing inputs before calling execute. GCP Cloud Run was this same pool at concurrency 1 behind one `/execute` endpoint — that remote pool no longer exists.
