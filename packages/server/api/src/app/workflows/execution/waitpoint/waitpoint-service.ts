...
export const waitpointService = (log: FastifyBaseLogger) => ({
    ...
    async handleResumeSignal(params: HandleResumeSignalParams): Promise<boolean> {
        ...
        if (executionStatus === ExecutionStatus.RUNNING || executionStatus === ExecutionStatus.QUEUED) {
            ...
            log.info({ execution: { id: executionId } }, '[waitpointService#handleResumeSignal] Marked PENDING waitpoint COMPLETED while workflow still RUNNING/QUEUED; runsMetadataQueue will trigger resume on PAUSED upload')
            return true
        }

        log.info({ execution: { id: executionId }, executionStatus }, '[waitpointService#handleResumeSignal] Workflow run not in resumable state, ignoring')
        return false
    },
